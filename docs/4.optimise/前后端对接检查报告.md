...
1. **移除 kuaizhizao 中的 BOM 管理页面**
   ...
   - 更新路由配置，移除 BOM 路由
   - 如需在 kuaizhizao 中访问 BOM，改为跳转到 `master-data` 的 BOM 页面

2. **移除 kuaizhizao 中的业务配置页面**
   - 删除 `apps/kuaizhizao/pages/common/business-config/`
   - 更新 manifest.json，移除业务配置菜单项
   - 更新路由配置，移除业务配置路由
   - 系统参数和编码规则应在系统级菜单中提供入口

3. **更新系统级菜单**
   - 在系统级菜单中添加"系统参数"和"编码规则"管理入口
   - 确保系统级功能对组织管理员可见

#### 中优先级

4. **清理前端服务定义**
   - 移除 `apps/kuaizhizao/services/common.ts` 中的系统参数和编码规则API（如果使用系统级API）
   - 移除 `apps/kuaizhizao/services/production.ts` 中的 `bomApi`（改为从 `master-data` 导入）
