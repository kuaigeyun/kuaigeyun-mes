...
async def list_licenses(
    ...
    _auth=Depends(require_permission_codes("kuaioa:license:read")),
...
async def create_license(
    ...
    _auth=Depends(require_permission_codes("kuaioa:license:create")),
    tenant_id: int = Depends(get_current_tenant),
):
...
async def update_license(
    ...
    license_id: int = Path(..., ge=1),
    current_user: User = Depends(get_current_user),
    _auth=Depends(require_permission_codes("kuaioa:license:update")),
    tenant_id: int = Depends(get_current_tenant),
):
    try:
...
async def delete_license(
    license_id: int = Path(..., ge=1),
    current_user: User = Depends(get_current_user),
    _auth=Depends(require_permission_codes("kuaioa:license:delete")),
    tenant_id: int = Depends(get_current_tenant),
):
    try:
