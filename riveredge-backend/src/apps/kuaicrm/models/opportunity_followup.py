...
class OpportunityFollowUp(BaseModel):
    ...
    Attributes:
        ...
        opportunity_id: 商机ID（关联Opportunity）
        followup_type: 跟进类型（电话、邮件、拜访、会议、演示等）
    ...
    # 关联商机
    opportunity_id = fields.IntField(description="商机ID（关联Opportunity）")
    
    # 跟进信息
    followup_type = fields.CharField(max_length=50, description="跟进类型（电话、邮件、拜访、会议、演示等）")
    ...
    def __str__(self):
        """字符串表示"""
        return f"OpportunityFollowUp {self.id} - {self.followup_type}"
