...
class CostRuleBase(BaseModel):
    ...
    allocation_basis: Optional[str] = Field(None, description="分摊基准（如：产量、工时、机器工时、产值、手动分摊）")
    wip_valuation_method: Optional[str] = Field(None, description="在产品核算方法（如：不计算、约当产量法、定额成本法、只计材料）")
    source_module: Optional[str] = Field(None, description="费用来源模块（如：薪资、采购、仓库、报工）")
...
class CostRuleUpdate(BaseModel):
    ...
    allocation_basis: Optional[str] = Field(None, description="分摊基准")
    wip_valuation_method: Optional[str] = Field(None, description="在产品核算方法")
    source_module: Optional[str] = Field(None, description="费用来源模块")
    calculation_formula: Optional[Dict[str, Any]] = Field(None, description="计算公式（JSON格式）")
    rule_parameters: Optional[Dict[str, Any]] = Field(None, description="规则参数（JSON格式）")
