"""将设备卡打印模板同步为可视化设计器版（100×70mm 单卡）。

仅更新 EQUIPMENT_CARD_PRINT / EQUIPMENT_CARD_PRINT_*。

用法:
  cd riveredge-backend && PYTHONPATH=src python -m apps.kuaizhizao.scripts.sync_equipment_card_print_template
"""
...
async def sync() -> int:
    ...
    try:
        preset = build_equipment_card_preset()
        rows = await PrintTemplate.filter(deleted_at__isnull=True).all()
        updated = 0
        for row in rows:
            code = (row.code or "").strip().upper()
            if code != "EQUIPMENT_CARD_PRINT" and not code.startswith("EQUIPMENT_CARD_PRINT_"):
                continue
