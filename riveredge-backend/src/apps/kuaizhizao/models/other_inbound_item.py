...
class OtherInboundItem(BaseModel):
    """
    其他入库单明细
    ...
    inbound_id = fields.IntField(description="入库单ID")
    ...
    class Meta:
        table = "apps_kuaizhizao_other_inbound_items"
        table_description = "快格轻制造 - 其他入库单明细"
        indexes = [
            ("tenant_id", "inbound_id"),
            ("material_id",),
            ("location_id",),
        ]
