...
def _clip_completion(v: str, *, label: str) -> str:
    s = v.strip()
    if not s:
...
class EquipmentUpkeepCompleteSheetOut(BaseModel):
    ...
    header_attachment_file_uuids: List[str] = Field(default_factory=list, description="保养后附件")
    source_header_attachment_file_uuids: List[str] = Field(
        default_factory=list,
        description="来源保养单表头附件·保养前（只读对比）",
    )
...
class EquipmentUpkeepCompleteSheetCreate(BaseModel):
    ...
    applicant_user_id: Optional[int] = Field(None, ge=1, description="缺省从来源保养单带出")
    department_uuid: Optional[str] = Field(None, max_length=36, description="缺省从来源保养单带出")
    header_attachment_file_uuids: Optional[List[str]] = Field(None, description="保养后附件")
    completion_content: str = Field(min_length=1, description="保养完成说明")

    @field_validator("completion_content", mode="before")
    @classmethod
    def strip_completion(cls, v):
        if v is None:
            raise ValueError("请填写保养完成说明")
        s = str(v).strip()
        if not s:
            raise ValueError("请填写保养完成说明")
        return s
...
@router.get("", summary="设备保养完成单分页列表")
...
@router.post("", response_model=EquipmentUpkeepCompleteSheetOut, summary="创建设备保养完成单")
async def create_upkeep_complete_sheet(
    ...
    if not src:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="设备保养单不存在")
    if await tenant_alive(HaoligoEquipmentUpkeepCompleteSheet, tenant_id).filter(
        source_upkeep_sheet_id=src.id,
        deleted_at__isnull=True,
    ).exists():
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="该保养单已存在保养完成单，不可重复确认")
    src_no = (str(src.sheet_no or "").strip() or f"保养单#{src.id}")
    ...
    completion = _clip_completion(body.completion_content, label="保养完成说明")
...
@router.get("/{row_id}", response_model=EquipmentUpkeepCompleteSheetOut, summary="设备保养完成单详情")
...
@router.patch("/{row_id}", response_model=EquipmentUpkeepCompleteSheetOut, summary="更新设备保养完成单")
async def update_upkeep_complete_sheet(
    ...
    if "completion_content" in data and data["completion_content"] is not None:
        data["completion_content"] = _clip_completion(str(data["completion_content"]), label="保养完成说明")
...
@router.delete("/{row_id}", status_code=status.HTTP_204_NO_CONTENT, summary="软删除设备保养完成单")
