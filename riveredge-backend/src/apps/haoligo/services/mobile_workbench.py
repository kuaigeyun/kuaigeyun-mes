...
def _perm_set_grants_review(user_perms: set[str], resource_prefix: str) -> bool:
    app_code, module_code = _parse_resource_prefix(resource_prefix)
    codes = review_permission_codes(app_code, module_code)
    return _perm_set_overlaps(user_perms, codes)
...
def _entry_visible(
    ...
    *,
    ...
    if review_resource:
        return _perm_set_grants_review(user_perms, review_resource)
