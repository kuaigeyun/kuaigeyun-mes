...
class PartnerStatement(BaseModel):
    ...
    debit_total = fields.DecimalField(max_digits=14, decimal_places=2, description="本期借方总额 (发生应收/付)")
    credit_total = fields.DecimalField(max_digits=14, decimal_places=2, description="本期贷方总额 (发生实收/付)")
    ...
    # 对账明细 (JSON 存储或通过查询生成，此处存快照)
    transaction_details = fields.JSONField(null=True, description="往来明细快照")

    confirmed_at = fields.DatetimeField(null=True, description="双方确认时间")
