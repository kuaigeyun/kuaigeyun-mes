...
class Package(BaseModel):
    ...
    max_branch_organizations = fields.IntField(null=True, description="最大分支组织数量限制（null=不限制）")
    is_active = fields.BooleanField(default=True, description="是否激活")
    allow_pro_apps = fields.BooleanField(default=False, description="是否允许使用 PRO 应用")
    allowed_app_codes = fields.JSONField(default=list, description="允许加载的应用编码白名单（空=不限制）")
