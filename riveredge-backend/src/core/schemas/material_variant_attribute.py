...
class MaterialVariantAttributeDefinitionBase(BaseModel):
    ...
    allow_multiple: bool = Field(default=False, description="枚举类型是否允许多选（仅 attribute_type=enum 时有效）")
...
class MaterialVariantAttributeDefinitionUpdate(BaseModel):
    ...
    allow_multiple: Optional[bool] = Field(None, description="枚举类型是否允许多选")
