...
为数据库表添加缺失的外键约束，提升数据完整性。

变更内容：
- 系统级 (core_*) 表的外键约束
- 应用级主数据 (apps_master_data_*) 表的外键约束
- 使用幂等性检查，如果外键已存在则跳过
...
async def upgrade(db: BaseDBAsyncClient) -> str:
    ...
    return """
        ...
        DO $$
        BEGIN
            -- department_id -> core_departments.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_users_department_id' 
                AND conrelid = 'core_users'::regclass
            ) THEN
                ALTER TABLE "core_users" 
                ADD CONSTRAINT "fk_core_users_department_id" 
                FOREIGN KEY ("department_id") 
                REFERENCES "core_departments"("id") 
                ON DELETE SET NULL;
            END IF;
            
            -- position_id -> core_positions.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_users_position_id' 
                AND conrelid = 'core_users'::regclass
            ) THEN
                ALTER TABLE "core_users" 
                ADD CONSTRAINT "fk_core_users_position_id" 
                FOREIGN KEY ("position_id") 
                REFERENCES "core_positions"("id") 
                ON DELETE SET NULL;
            END IF;
        END $$;
        ...
        DO $$
        BEGIN
            -- parent_id -> core_departments.id (ON DELETE SET NULL, 自引用)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_departments_parent_id' 
                AND conrelid = 'core_departments'::regclass
            ) THEN
                ALTER TABLE "core_departments" 
                ADD CONSTRAINT "fk_core_departments_parent_id" 
                FOREIGN KEY ("parent_id") 
                REFERENCES "core_departments"("id") 
                ON DELETE SET NULL;
            END IF;
            
            -- manager_id -> core_users.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_departments_manager_id' 
                AND conrelid = 'core_departments'::regclass
            ) THEN
                ALTER TABLE "core_departments" 
                ADD CONSTRAINT "fk_core_departments_manager_id" 
        ...
        BEGIN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_positions_department_id' 
                AND conrelid = 'core_positions'::regclass
            ) THEN
                ALTER TABLE "core_positions" 
                ADD CONSTRAINT "fk_core_positions_department_id" 
                FOREIGN KEY ("department_id") 
                REFERENCES "core_departments"("id") 
                ON DELETE SET NULL;
            END IF;
        ...
        DO $$
        BEGIN
            -- parent_id -> core_menus.id (ON DELETE SET NULL, 自引用)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_menus_parent_id' 
                AND conrelid = 'core_menus'::regclass
            ) THEN
                ALTER TABLE "core_menus" 
                ADD CONSTRAINT "fk_core_menus_parent_id" 
                FOREIGN KEY ("parent_id") 
                REFERENCES "core_menus"("id") 
                ON DELETE SET NULL;
            END IF;
        END $$;
        ...
        DO $$
        BEGIN
            -- data_source_id -> core_data_sources.id (ON DELETE RESTRICT)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_datasets_data_source_id' 
                AND conrelid = 'core_datasets'::regclass
            ) THEN
                ALTER TABLE "core_datasets" 
                ADD CONSTRAINT "fk_core_datasets_data_source_id" 
                FOREIGN KEY ("data_source_id") 
                REFERENCES "core_data_sources"("id") 
                ON DELETE RESTRICT;
            END IF;
        END $$;
        
        -- core_approval_instances 表
        DO $$
        BEGIN
            -- process_id -> core_approval_processes.id (ON DELETE RESTRICT)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_approval_instances_process_id' 
                AND conrelid = 'core_approval_instances'::regclass
            ) THEN
                ALTER TABLE "core_approval_instances" 
                ADD CONSTRAINT "fk_core_approval_instances_process_id" 
                FOREIGN KEY ("process_id") 
                REFERENCES "core_approval_processes"("id") 
                ON DELETE RESTRICT;
            END IF;
        END $$;
        
        -- core_dictionary_items 表
        DO $$
        BEGIN
            -- dictionary_id -> core_data_dictionaries.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_dictionary_items_dictionary_id' 
                AND conrelid = 'core_dictionary_items'::regclass
            ) THEN
                ALTER TABLE "core_dictionary_items" 
                ADD CONSTRAINT "fk_core_dictionary_items_dictionary_id" 
                FOREIGN KEY ("dictionary_id") 
                REFERENCES "core_data_dictionaries"("id") 
                ON DELETE CASCADE;
            END IF;
        END $$;
        
        -- core_code_sequences 表
        DO $$
        BEGIN
            -- code_rule_id -> core_code_rules.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_code_sequences_code_rule_id' 
                AND conrelid = 'core_code_sequences'::regclass
            ) THEN
                ALTER TABLE "core_code_sequences" 
                ADD CONSTRAINT "fk_core_code_sequences_code_rule_id" 
                FOREIGN KEY ("code_rule_id") 
                REFERENCES "core_code_rules"("id") 
                ON DELETE CASCADE;
            END IF;
        END $$;
        
        -- core_custom_field_values 表
        DO $$
        BEGIN
            -- custom_field_id -> core_custom_fields.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_custom_field_values_custom_field_id' 
                AND conrelid = 'core_custom_field_values'::regclass
            ) THEN
                ALTER TABLE "core_custom_field_values" 
                ADD CONSTRAINT "fk_core_custom_field_values_custom_field_id" 
                FOREIGN KEY ("custom_field_id") 
                REFERENCES "core_custom_fields"("id") 
                ON DELETE CASCADE;
            END IF;
        END $$;
        
        -- core_role_permissions 表
        DO $$
        BEGIN
            -- role_id -> core_roles.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_role_permissions_role_id' 
                AND conrelid = 'core_role_permissions'::regclass
            ) THEN
                ALTER TABLE "core_role_permissions" 
                ADD CONSTRAINT "fk_core_role_permissions_role_id" 
                FOREIGN KEY ("role_id") 
                REFERENCES "core_roles"("id") 
                ON DELETE CASCADE;
            END IF;
            
            -- permission_id -> core_permissions.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_role_permissions_permission_id' 
                AND conrelid = 'core_role_permissions'::regclass
            ) THEN
                ALTER TABLE "core_role_permissions" 
                ADD CONSTRAINT "fk_core_role_permissions_permission_id" 
        ...
        DO $$
        BEGIN
            -- user_id -> core_users.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_user_roles_user_id' 
                AND conrelid = 'core_user_roles'::regclass
            ) THEN
                ALTER TABLE "core_user_roles" 
                ADD CONSTRAINT "fk_core_user_roles_user_id" 
                FOREIGN KEY ("user_id") 
                REFERENCES "core_users"("id") 
                ON DELETE CASCADE;
            END IF;
            ...
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_user_roles_role_id' 
                AND conrelid = 'core_user_roles'::regclass
            ) THEN
                ALTER TABLE "core_user_roles" 
                ADD CONSTRAINT "fk_core_user_roles_role_id" 
                FOREIGN KEY ("role_id") 
                REFERENCES "core_roles"("id") 
                ON DELETE CASCADE;
            END IF;
        ...
        -- core_approval_histories 表
        DO $$
        BEGIN
            -- approval_instance_id -> core_approval_instances.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_approval_histories_approval_instance_id' 
                AND conrelid = 'core_approval_histories'::regclass
            ) THEN
                ALTER TABLE "core_approval_histories" 
                ADD CONSTRAINT "fk_core_approval_histories_approval_instance_id" 
                FOREIGN KEY ("approval_instance_id") 
                REFERENCES "core_approval_instances"("id") 
                ON DELETE CASCADE;
            END IF;
            
            -- action_by -> core_users.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_approval_histories_action_by' 
                AND conrelid = 'core_approval_histories'::regclass
            ) THEN
                ALTER TABLE "core_approval_histories" 
                ADD CONSTRAINT "fk_core_approval_histories_action_by" 
        ...
        -- core_saved_searches 表
        DO $$
        BEGIN
            -- user_id -> core_users.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_saved_searches_user_id' 
                AND conrelid = 'core_saved_searches'::regclass
            ) THEN
                ALTER TABLE "core_saved_searches" 
                ADD CONSTRAINT "fk_core_saved_searches_user_id" 
                FOREIGN KEY ("user_id") 
                REFERENCES "core_users"("id") 
                ON DELETE CASCADE;
            END IF;
        ...
        -- core_user_preferences 表
        DO $$
        BEGIN
            -- user_id -> core_users.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_core_user_preferences_user_id' 
                AND conrelid = 'core_user_preferences'::regclass
            ) THEN
                ALTER TABLE "core_user_preferences" 
                ADD CONSTRAINT "fk_core_user_preferences_user_id" 
                FOREIGN KEY ("user_id") 
                REFERENCES "core_users"("id") 
                ON DELETE CASCADE;
            END IF;
        ...
        -- apps_master_data_materials 表
        DO $$
        BEGIN
            -- group_id -> apps_master_data_material_groups.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_master_data_materials_group_id' 
                AND conrelid = 'apps_master_data_materials'::regclass
            ) THEN
                ALTER TABLE "apps_master_data_materials" 
                ADD CONSTRAINT "fk_apps_master_data_materials_group_id" 
                FOREIGN KEY ("group_id") 
                REFERENCES "apps_master_data_material_groups"("id") 
                ON DELETE SET NULL;
            END IF;
        END $$;
        
        -- apps_master_data_bom 表
        DO $$
        BEGIN
            -- material_id -> apps_master_data_materials.id (ON DELETE RESTRICT)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_master_data_bom_material_id' 
                AND conrelid = 'apps_master_data_bom'::regclass
            ) THEN
            ...
            -- component_id -> apps_master_data_materials.id (ON DELETE RESTRICT)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_master_data_bom_component_id' 
                AND conrelid = 'apps_master_data_bom'::regclass
            ) THEN
                ALTER TABLE "apps_master_data_bom" 
        ...
        -- apps_master_data_storage_areas 表
        DO $$
        BEGIN
            -- warehouse_id -> apps_master_data_warehouses.id (ON DELETE RESTRICT)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_master_data_storage_areas_warehouse_id' 
                AND conrelid = 'apps_master_data_storage_areas'::regclass
            ) THEN
                ALTER TABLE "apps_master_data_storage_areas" 
                ADD CONSTRAINT "fk_apps_master_data_storage_areas_warehouse_id" 
                FOREIGN KEY ("warehouse_id") 
                REFERENCES "apps_master_data_warehouses"("id") 
                ON DELETE RESTRICT;
            END IF;
        END $$;
        
        -- apps_master_data_storage_locations 表
        DO $$
        BEGIN
            -- storage_area_id -> apps_master_data_storage_areas.id (ON DELETE RESTRICT)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_master_data_storage_locations_storage_area_id' 
                AND conrelid = 'apps_master_data_storage_locations'::regclass
            ) THEN
                ALTER TABLE "apps_master_data_storage_locations" 
                ADD CONSTRAINT "fk_apps_master_data_storage_locations_storage_area_id" 
                FOREIGN KEY ("storage_area_id") 
                REFERENCES "apps_master_data_storage_areas"("id") 
                ON DELETE RESTRICT;
            END IF;
        END $$;
        
        -- apps_master_data_production_lines 表
        DO $$
        BEGIN
            -- workshop_id -> apps_master_data_workshops.id (ON DELETE RESTRICT)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_master_data_production_lines_workshop_id' 
                AND conrelid = 'apps_master_data_production_lines'::regclass
            ) THEN
                ALTER TABLE "apps_master_data_production_lines" 
                ADD CONSTRAINT "fk_apps_master_data_production_lines_workshop_id" 
                FOREIGN KEY ("workshop_id") 
                REFERENCES "apps_master_data_workshops"("id") 
                ON DELETE RESTRICT;
            END IF;
        END $$;
        
        -- apps_master_data_workstations 表
        DO $$
        BEGIN
            -- production_line_id -> apps_master_data_production_lines.id (ON DELETE RESTRICT)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_master_data_workstations_production_line_id' 
                AND conrelid = 'apps_master_data_workstations'::regclass
            ) THEN
                ALTER TABLE "apps_master_data_workstations" 
                ADD CONSTRAINT "fk_apps_master_data_workstations_production_line_id" 
                FOREIGN KEY ("production_line_id") 
                REFERENCES "apps_master_data_production_lines"("id") 
                ON DELETE RESTRICT;
            END IF;
        END $$;
        
        -- apps_master_data_sop_executions 表
        DO $$
        BEGIN
            -- sop_id -> apps_master_data_sop.id (ON DELETE RESTRICT)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_master_data_sop_executions_sop_id' 
                AND conrelid = 'apps_master_data_sop_executions'::regclass
            ) THEN
                ALTER TABLE "apps_master_data_sop_executions" 
                ADD CONSTRAINT "fk_apps_master_data_sop_executions_sop_id" 
                FOREIGN KEY ("sop_id") 
                REFERENCES "apps_master_data_sop"("id") 
                ON DELETE RESTRICT;
            END IF;
        END $$;
    """
...
async def downgrade(db: BaseDBAsyncClient) -> str:
    ...
    return """
        ...
        ALTER TABLE IF EXISTS "apps_master_data_sop_executions" 
            DROP CONSTRAINT IF EXISTS "fk_apps_master_data_sop_executions_sop_id";
        
        ALTER TABLE IF EXISTS "apps_master_data_workstations" 
            DROP CONSTRAINT IF EXISTS "fk_apps_master_data_workstations_production_line_id";
        
        ALTER TABLE IF EXISTS "apps_master_data_production_lines" 
            DROP CONSTRAINT IF EXISTS "fk_apps_master_data_production_lines_workshop_id";
        
        ALTER TABLE IF EXISTS "apps_master_data_storage_locations" 
            DROP CONSTRAINT IF EXISTS "fk_apps_master_data_storage_locations_storage_area_id";
        
        ALTER TABLE IF EXISTS "apps_master_data_storage_areas" 
            DROP CONSTRAINT IF EXISTS "fk_apps_master_data_storage_areas_warehouse_id";
        
        ALTER TABLE IF EXISTS "apps_master_data_bom" 
            DROP CONSTRAINT IF EXISTS "fk_apps_master_data_bom_component_id";
        
        ALTER TABLE IF EXISTS "apps_master_data_bom" 
            DROP CONSTRAINT IF EXISTS "fk_apps_master_data_bom_material_id";
        
        ALTER TABLE IF EXISTS "apps_master_data_materials" 
            DROP CONSTRAINT IF EXISTS "fk_apps_master_data_materials_group_id";
        
        -- ============================================
        -- 删除系统级表的外键约束
        -- ============================================
        ALTER TABLE IF EXISTS "core_user_preferences" 
            DROP CONSTRAINT IF EXISTS "fk_core_user_preferences_user_id";
        
        ALTER TABLE IF EXISTS "core_saved_searches" 
            DROP CONSTRAINT IF EXISTS "fk_core_saved_searches_user_id";
        
        ALTER TABLE IF EXISTS "core_approval_histories" 
            DROP CONSTRAINT IF EXISTS "fk_core_approval_histories_action_by";
        
        ALTER TABLE IF EXISTS "core_approval_histories" 
            DROP CONSTRAINT IF EXISTS "fk_core_approval_histories_approval_instance_id";
        
        ALTER TABLE IF EXISTS "core_user_roles" 
            DROP CONSTRAINT IF EXISTS "fk_core_user_roles_role_id";
        
        ALTER TABLE IF EXISTS "core_user_roles" 
            DROP CONSTRAINT IF EXISTS "fk_core_user_roles_user_id";
        
        ALTER TABLE IF EXISTS "core_role_permissions" 
            DROP CONSTRAINT IF EXISTS "fk_core_role_permissions_permission_id";
        
        ALTER TABLE IF EXISTS "core_role_permissions" 
            DROP CONSTRAINT IF EXISTS "fk_core_role_permissions_role_id";
        
        ALTER TABLE IF EXISTS "core_custom_field_values" 
            DROP CONSTRAINT IF EXISTS "fk_core_custom_field_values_custom_field_id";
        
        ALTER TABLE IF EXISTS "core_code_sequences" 
            DROP CONSTRAINT IF EXISTS "fk_core_code_sequences_code_rule_id";
        
        ALTER TABLE IF EXISTS "core_dictionary_items" 
            DROP CONSTRAINT IF EXISTS "fk_core_dictionary_items_dictionary_id";
        
        ALTER TABLE IF EXISTS "core_approval_instances" 
            DROP CONSTRAINT IF EXISTS "fk_core_approval_instances_process_id";
        
        ALTER TABLE IF EXISTS "core_datasets" 
            DROP CONSTRAINT IF EXISTS "fk_core_datasets_data_source_id";
        
        ALTER TABLE IF EXISTS "core_menus" 
            DROP CONSTRAINT IF EXISTS "fk_core_menus_parent_id";
        
        ALTER TABLE IF EXISTS "core_positions" 
            DROP CONSTRAINT IF EXISTS "fk_core_positions_department_id";
        
        ALTER TABLE IF EXISTS "core_departments" 
            DROP CONSTRAINT IF EXISTS "fk_core_departments_manager_id";
        
        ALTER TABLE IF EXISTS "core_departments" 
            DROP CONSTRAINT IF EXISTS "fk_core_departments_parent_id";
        
        ALTER TABLE IF EXISTS "core_users" 
            DROP CONSTRAINT IF EXISTS "fk_core_users_position_id";
        
        ALTER TABLE IF EXISTS "core_users" 
            DROP CONSTRAINT IF EXISTS "fk_core_users_department_id";
    """
