...
async def upgrade(db: BaseDBAsyncClient) -> str:
    ...
    return """
        ...
        -- 清理 apps_kuaizhizao_work_orders 表中的孤立记录
        UPDATE "apps_kuaizhizao_work_orders" 
        SET "sales_order_id" = NULL 
        WHERE "sales_order_id" IS NOT NULL 
        AND "sales_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_sales_orders");
        
        -- 清理 apps_kuaizhizao_sales_order_items 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_sales_order_items" 
        WHERE "sales_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_sales_orders");
        
        UPDATE "apps_kuaizhizao_sales_order_items" 
        SET "work_order_id" = NULL 
        WHERE "work_order_id" IS NOT NULL 
        AND "work_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_work_orders");
        
        -- 清理 apps_kuaizhizao_reporting_records 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_reporting_records" 
        WHERE "work_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_work_orders");
        
        -- 清理 apps_kuaizhizao_production_pickings 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_production_pickings" 
        WHERE "work_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_work_orders");
        
        -- 清理 apps_kuaizhizao_production_picking_items 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_production_picking_items" 
        WHERE "picking_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_production_pickings");
        
        -- 清理 apps_kuaizhizao_process_inspections 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_process_inspections" 
        WHERE "work_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_work_orders");
        
        -- 清理 apps_kuaizhizao_finished_goods_receipts 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_finished_goods_receipts" 
        WHERE "work_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_work_orders");
        ...
        -- 清理 apps_kuaizhizao_finished_goods_receipt_items 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_finished_goods_receipt_items" 
        WHERE "receipt_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_finished_goods_receipts");
        
        -- 清理 apps_kuaizhizao_finished_goods_inspections 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_finished_goods_inspections" 
        WHERE "work_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_work_orders");
        ...
        -- 清理 apps_kuaizhizao_sales_deliveries 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_sales_deliveries" 
        WHERE "sales_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_sales_orders");
        
        -- 清理 apps_kuaizhizao_sales_delivery_items 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_sales_delivery_items" 
        WHERE "delivery_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_sales_deliveries");
        
        -- 清理 apps_kuaizhizao_purchase_receipts 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_purchase_receipts" 
        WHERE "purchase_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_purchase_orders");
        
        -- 清理 apps_kuaizhizao_purchase_receipt_items 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_purchase_receipt_items" 
        WHERE "receipt_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_purchase_receipts");
        
        DELETE FROM "apps_kuaizhizao_purchase_receipt_items" 
        WHERE "purchase_order_item_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_purchase_order_items");
        
        -- 清理 apps_kuaizhizao_incoming_inspections 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_incoming_inspections" 
        WHERE "purchase_receipt_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_purchase_receipts");
        
        -- 清理 apps_kuaizhizao_purchase_invoices 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_purchase_invoices" 
        WHERE "purchase_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_purchase_orders");
        
        UPDATE "apps_kuaizhizao_purchase_invoices" 
        SET "payable_id" = NULL 
        WHERE "payable_id" IS NOT NULL 
        AND "payable_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_payables");
        
        -- 清理 apps_kuaizhizao_production_plans 表中的孤立记录
        UPDATE "apps_kuaizhizao_production_plans" 
        SET "source_id" = NULL 
        WHERE "source_id" IS NOT NULL 
        AND "source_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_sales_forecasts");
        
        -- 清理 apps_kuaizhizao_production_plan_items 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_production_plan_items" 
        WHERE "plan_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_production_plans");
        
        UPDATE "apps_kuaizhizao_production_plan_items" 
        SET "work_order_id" = NULL 
        WHERE "work_order_id" IS NOT NULL 
        AND "work_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_work_orders");
        ...
        AND "purchase_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_purchase_orders");
        
        -- 清理 apps_kuaizhizao_sales_forecast_items 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_sales_forecast_items" 
        WHERE "forecast_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_sales_forecasts");
        
        -- 清理 apps_kuaizhizao_mrp_results 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_mrp_results" 
        WHERE "forecast_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_sales_forecasts");
        
        -- 清理 apps_kuaizhizao_lrp_results 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_lrp_results" 
        WHERE "sales_order_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_sales_orders");
        
        -- 清理 apps_kuaizhizao_bill_of_materials_items 表中的孤立记录
        DELETE FROM "apps_kuaizhizao_bill_of_materials_items" 
        WHERE "bom_id" NOT IN (SELECT "id" FROM "apps_kuaizhizao_bill_of_materials");
        
        -- ============================================
        -- 添加快格轻制造表的外键约束
        -- ============================================
        
        -- apps_kuaizhizao_work_orders 表
        DO $$
        BEGIN
            -- sales_order_id -> apps_kuaizhizao_sales_orders.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_work_orders_sales_order_id' 
                AND conrelid = 'apps_kuaizhizao_work_orders'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_work_orders" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_work_orders_sales_order_id" 
        ...
        -- apps_kuaizhizao_sales_order_items 表
        DO $$
        BEGIN
            -- sales_order_id -> apps_kuaizhizao_sales_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_sales_order_items_sales_order_id' 
                AND conrelid = 'apps_kuaizhizao_sales_order_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_sales_order_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_sales_order_items_sales_order_id" 
                FOREIGN KEY ("sales_order_id") 
                REFERENCES "apps_kuaizhizao_sales_orders"("id") 
                ON DELETE CASCADE;
            END IF;
            
            -- work_order_id -> apps_kuaizhizao_work_orders.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_sales_order_items_work_order_id' 
                AND conrelid = 'apps_kuaizhizao_sales_order_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_sales_order_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_sales_order_items_work_order_id" 
                FOREIGN KEY ("work_order_id") 
                REFERENCES "apps_kuaizhizao_work_orders"("id") 
                ON DELETE SET NULL;
            END IF;
        END $$;
        
        -- apps_kuaizhizao_reporting_records 表
        DO $$
        BEGIN
            -- work_order_id -> apps_kuaizhizao_work_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_reporting_records_work_order_id' 
                AND conrelid = 'apps_kuaizhizao_reporting_records'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_reporting_records" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_reporting_records_work_order_id" 
                FOREIGN KEY ("work_order_id") 
                REFERENCES "apps_kuaizhizao_work_orders"("id") 
                ON DELETE CASCADE;
            END IF;
        ...
        -- apps_kuaizhizao_production_pickings 表
        DO $$
        BEGIN
            -- work_order_id -> apps_kuaizhizao_work_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_production_pickings_work_order_id' 
                AND conrelid = 'apps_kuaizhizao_production_pickings'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_production_pickings" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_production_pickings_work_order_id" 
                FOREIGN KEY ("work_order_id") 
                REFERENCES "apps_kuaizhizao_work_orders"("id") 
                ON DELETE CASCADE;
            END IF;
        ...
        -- apps_kuaizhizao_production_picking_items 表
        DO $$
        BEGIN
            -- picking_id -> apps_kuaizhizao_production_pickings.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_production_picking_items_picking_id' 
                AND conrelid = 'apps_kuaizhizao_production_picking_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_production_picking_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_production_picking_items_picking_id" 
                FOREIGN KEY ("picking_id") 
                REFERENCES "apps_kuaizhizao_production_pickings"("id") 
                ON DELETE CASCADE;
            END IF;
        END $$;
        
        -- apps_kuaizhizao_process_inspections 表
        DO $$
        BEGIN
            -- work_order_id -> apps_kuaizhizao_work_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_process_inspections_work_order_id' 
                AND conrelid = 'apps_kuaizhizao_process_inspections'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_process_inspections" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_process_inspections_work_order_id" 
                FOREIGN KEY ("work_order_id") 
                REFERENCES "apps_kuaizhizao_work_orders"("id") 
                ON DELETE CASCADE;
            END IF;
        ...
        -- apps_kuaizhizao_finished_goods_receipts 表
        DO $$
        BEGIN
            -- work_order_id -> apps_kuaizhizao_work_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_finished_goods_receipts_work_order_id' 
                AND conrelid = 'apps_kuaizhizao_finished_goods_receipts'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_finished_goods_receipts" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_finished_goods_receipts_work_order_id" 
                FOREIGN KEY ("work_order_id") 
                REFERENCES "apps_kuaizhizao_work_orders"("id") 
                ON DELETE CASCADE;
            END IF;
            ...
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_finished_goods_receipts_sales_order_id' 
                AND conrelid = 'apps_kuaizhizao_finished_goods_receipts'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_finished_goods_receipts" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_finished_goods_receipts_sales_order_id" 
                FOREIGN KEY ("sales_order_id") 
                REFERENCES "apps_kuaizhizao_sales_orders"("id") 
                ON DELETE SET NULL;
            END IF;
        ...
        -- apps_kuaizhizao_finished_goods_receipt_items 表
        DO $$
        BEGIN
            -- receipt_id -> apps_kuaizhizao_finished_goods_receipts.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_finished_goods_receipt_items_receipt_id' 
                AND conrelid = 'apps_kuaizhizao_finished_goods_receipt_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_finished_goods_receipt_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_finished_goods_receipt_items_receipt_id" 
                FOREIGN KEY ("receipt_id") 
                REFERENCES "apps_kuaizhizao_finished_goods_receipts"("id") 
                ON DELETE CASCADE;
            END IF;
        END $$;
        
        -- apps_kuaizhizao_finished_goods_inspections 表
        DO $$
        BEGIN
            -- work_order_id -> apps_kuaizhizao_work_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_finished_goods_inspections_work_order_id' 
                AND conrelid = 'apps_kuaizhizao_finished_goods_inspections'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_finished_goods_inspections" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_finished_goods_inspections_work_order_id" 
                FOREIGN KEY ("work_order_id") 
                REFERENCES "apps_kuaizhizao_work_orders"("id") 
                ON DELETE CASCADE;
            END IF;
            ...
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_finished_goods_inspections_sales_order_id' 
                AND conrelid = 'apps_kuaizhizao_finished_goods_inspections'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_finished_goods_inspections" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_finished_goods_inspections_sales_order_id" 
                FOREIGN KEY ("sales_order_id") 
                REFERENCES "apps_kuaizhizao_sales_orders"("id") 
                ON DELETE SET NULL;
            END IF;
        ...
        -- apps_kuaizhizao_sales_deliveries 表
        DO $$
        BEGIN
            -- sales_order_id -> apps_kuaizhizao_sales_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
        ...
        -- apps_kuaizhizao_sales_delivery_items 表
        DO $$
        BEGIN
            -- delivery_id -> apps_kuaizhizao_sales_deliveries.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_sales_delivery_items_delivery_id' 
                AND conrelid = 'apps_kuaizhizao_sales_delivery_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_sales_delivery_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_sales_delivery_items_delivery_id" 
                FOREIGN KEY ("delivery_id") 
                REFERENCES "apps_kuaizhizao_sales_deliveries"("id") 
                ON DELETE CASCADE;
            END IF;
        END $$;
        
        -- apps_kuaizhizao_purchase_receipts 表
        DO $$
        BEGIN
            -- purchase_order_id -> apps_kuaizhizao_purchase_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_purchase_receipts_purchase_order_id' 
                AND conrelid = 'apps_kuaizhizao_purchase_receipts'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_purchase_receipts" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_purchase_receipts_purchase_order_id" 
                FOREIGN KEY ("purchase_order_id") 
                REFERENCES "apps_kuaizhizao_purchase_orders"("id") 
                ON DELETE CASCADE;
            END IF;
        END $$;
        
        -- apps_kuaizhizao_purchase_receipt_items 表
        DO $$
        BEGIN
            -- receipt_id -> apps_kuaizhizao_purchase_receipts.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_purchase_receipt_items_receipt_id' 
                AND conrelid = 'apps_kuaizhizao_purchase_receipt_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_purchase_receipt_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_purchase_receipt_items_receipt_id" 
                FOREIGN KEY ("receipt_id") 
                REFERENCES "apps_kuaizhizao_purchase_receipts"("id") 
                ON DELETE CASCADE;
            END IF;
            
            -- purchase_order_item_id -> apps_kuaizhizao_purchase_order_items.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_purchase_receipt_items_purchase_order_item_id' 
                AND conrelid = 'apps_kuaizhizao_purchase_receipt_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_purchase_receipt_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_purchase_receipt_items_purchase_order_item_id" 
                FOREIGN KEY ("purchase_order_item_id") 
                REFERENCES "apps_kuaizhizao_purchase_order_items"("id") 
                ON DELETE SET NULL;
            END IF;
        END $$;
        
        -- apps_kuaizhizao_incoming_inspections 表
        DO $$
        BEGIN
            -- purchase_receipt_id -> apps_kuaizhizao_purchase_receipts.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_incoming_inspections_purchase_receipt_id' 
                AND conrelid = 'apps_kuaizhizao_incoming_inspections'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_incoming_inspections" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_incoming_inspections_purchase_receipt_id" 
        ...
        -- apps_kuaizhizao_purchase_invoices 表
        DO $$
        BEGIN
            -- purchase_order_id -> apps_kuaizhizao_purchase_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_purchase_invoices_purchase_order_id' 
                AND conrelid = 'apps_kuaizhizao_purchase_invoices'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_purchase_invoices" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_purchase_invoices_purchase_order_id" 
                FOREIGN KEY ("purchase_order_id") 
                REFERENCES "apps_kuaizhizao_purchase_orders"("id") 
                ON DELETE CASCADE;
            END IF;
            
            -- payable_id -> apps_kuaizhizao_payables.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_purchase_invoices_payable_id' 
                AND conrelid = 'apps_kuaizhizao_purchase_invoices'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_purchase_invoices" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_purchase_invoices_payable_id" 
                FOREIGN KEY ("payable_id") 
                REFERENCES "apps_kuaizhizao_payables"("id") 
                ON DELETE SET NULL;
            END IF;
        END $$;
        
        -- apps_kuaizhizao_production_plan_items 表
        DO $$
        BEGIN
            -- plan_id -> apps_kuaizhizao_production_plans.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_production_plan_items_plan_id' 
                AND conrelid = 'apps_kuaizhizao_production_plan_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_production_plan_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_production_plan_items_plan_id" 
                FOREIGN KEY ("plan_id") 
                REFERENCES "apps_kuaizhizao_production_plans"("id") 
                ON DELETE CASCADE;
            END IF;
            ...
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_production_plan_items_work_order_id' 
                AND conrelid = 'apps_kuaizhizao_production_plan_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_production_plan_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_production_plan_items_work_order_id" 
                FOREIGN KEY ("work_order_id") 
                REFERENCES "apps_kuaizhizao_work_orders"("id") 
                ON DELETE SET NULL;
            END IF;
            
            -- purchase_order_id -> apps_kuaizhizao_purchase_orders.id (ON DELETE SET NULL)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_production_plan_items_purchase_order_id' 
                AND conrelid = 'apps_kuaizhizao_production_plan_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_production_plan_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_production_plan_items_purchase_order_id" 
                FOREIGN KEY ("purchase_order_id") 
                REFERENCES "apps_kuaizhizao_purchase_orders"("id") 
                ON DELETE SET NULL;
            END IF;
        ...
        -- apps_kuaizhizao_sales_forecast_items 表
        DO $$
        BEGIN
            -- forecast_id -> apps_kuaizhizao_sales_forecasts.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_sales_forecast_items_forecast_id' 
                AND conrelid = 'apps_kuaizhizao_sales_forecast_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_sales_forecast_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_sales_forecast_items_forecast_id" 
                FOREIGN KEY ("forecast_id") 
                REFERENCES "apps_kuaizhizao_sales_forecasts"("id") 
                ON DELETE CASCADE;
            END IF;
        END $$;
        
        -- apps_kuaizhizao_mrp_results 表
        DO $$
        BEGIN
            -- forecast_id -> apps_kuaizhizao_sales_forecasts.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_mrp_results_forecast_id' 
                AND conrelid = 'apps_kuaizhizao_mrp_results'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_mrp_results" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_mrp_results_forecast_id" 
                FOREIGN KEY ("forecast_id") 
                REFERENCES "apps_kuaizhizao_sales_forecasts"("id") 
                ON DELETE CASCADE;
            END IF;
        ...
        -- apps_kuaizhizao_lrp_results 表
        DO $$
        BEGIN
            -- sales_order_id -> apps_kuaizhizao_sales_orders.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_lrp_results_sales_order_id' 
                AND conrelid = 'apps_kuaizhizao_lrp_results'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_lrp_results" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_lrp_results_sales_order_id" 
                FOREIGN KEY ("sales_order_id") 
                REFERENCES "apps_kuaizhizao_sales_orders"("id") 
                ON DELETE CASCADE;
            END IF;
        ...
        -- apps_kuaizhizao_bill_of_materials_items 表
        DO $$
        BEGIN
            -- bom_id -> apps_kuaizhizao_bill_of_materials.id (ON DELETE CASCADE)
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conname = 'fk_apps_kuaizhizao_bill_of_materials_items_bom_id' 
                AND conrelid = 'apps_kuaizhizao_bill_of_materials_items'::regclass
            ) THEN
                ALTER TABLE "apps_kuaizhizao_bill_of_materials_items" 
                ADD CONSTRAINT "fk_apps_kuaizhizao_bill_of_materials_items_bom_id" 
                FOREIGN KEY ("bom_id") 
                REFERENCES "apps_kuaizhizao_bill_of_materials"("id") 
                ON DELETE CASCADE;
            END IF;
        END $$;
    """
    ...
    return """
        ...
        BEGIN
            FOR r IN (
                ...
                FROM pg_constraint
                WHERE contype = 'f'
                AND conrelid::regclass::text LIKE 'apps_kuaizhizao_%'
            ) LOOP
