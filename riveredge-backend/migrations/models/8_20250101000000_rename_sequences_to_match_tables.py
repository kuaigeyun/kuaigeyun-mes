...
将序列名称统一为与表名匹配的命名规范，解决历史表名变更导致的序列命名不一致问题。

变更内容：
- seed_master_data_* → apps_master_data_* (应用级主数据表序列，去掉 seed_ 前缀，加上 apps_ 前缀)
- root_* → core_* (系统级表序列)
- sys_* → core_* (系统级表序列)
- soil_* → infra_* (平台级表序列)
...
async def upgrade(db: BaseDBAsyncClient) -> str:
    ...
    此迁移将重命名所有序列，使其与对应的表名保持一致。
    同时更新表定义中的序列引用。
    
    注意：如果目标序列已存在，将跳过重命名操作（说明已经正确）。
    """
    return """
        -- ============================================
        -- 序列重命名：seed_master_data_* → apps_master_data_*
        -- ============================================
        -- 应用级主数据表序列重命名（使用 DO 块进行条件检查）
        DO $$
        DECLARE
            old_seq_name TEXT;
            new_seq_name TEXT;
        BEGIN
            -- seed_master_data_bom_id_seq → apps_master_data_bom_id_seq
            old_seq_name := 'seed_master_data_bom_id_seq';
            new_seq_name := 'apps_master_data_bom_id_seq';
            IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'public' AND c.relname = old_seq_name AND c.relkind = 'S')
               AND NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'public' AND c.relname = new_seq_name AND c.relkind = 'S') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', old_seq_name, new_seq_name);
            END IF;
        END $$;
        ...
        DO $$
        BEGIN
            EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_customers_id_seq', 'apps_master_data_customers_id_seq');
        EXCEPTION
        ...
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_defect_types_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_defect_types_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_defect_types_id_seq', 'apps_master_data_defect_types_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_holidays_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_holidays_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_holidays_id_seq', 'apps_master_data_holidays_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_material_groups_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_material_groups_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_material_groups_id_seq', 'apps_master_data_material_groups_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_materials_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_materials_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_materials_id_seq', 'apps_master_data_materials_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_operations_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_operations_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_operations_id_seq', 'apps_master_data_operations_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_process_routes_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_process_routes_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_process_routes_id_seq', 'apps_master_data_process_routes_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_production_lines_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_production_lines_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_production_lines_id_seq', 'apps_master_data_production_lines_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_products_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_products_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_products_id_seq', 'apps_master_data_products_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_skills_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_skills_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_skills_id_seq', 'apps_master_data_skills_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_sop_executions_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_sop_executions_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_sop_executions_id_seq', 'apps_master_data_sop_executions_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_sop_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_sop_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_sop_id_seq', 'apps_master_data_sop_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_storage_areas_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_storage_areas_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_storage_areas_id_seq', 'apps_master_data_storage_areas_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_storage_locations_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_storage_locations_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_storage_locations_id_seq', 'apps_master_data_storage_locations_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_suppliers_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_suppliers_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_suppliers_id_seq', 'apps_master_data_suppliers_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_warehouses_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_warehouses_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_warehouses_id_seq', 'apps_master_data_warehouses_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_workshops_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_workshops_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_workshops_id_seq', 'apps_master_data_workshops_id_seq');
            END IF;
        END $$;
        
        DO $$
        BEGIN
            IF EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'seed_master_data_workstations_id_seq')
               AND NOT EXISTS (SELECT 1 FROM pg_sequences WHERE schemaname = 'public' AND sequencename = 'apps_master_data_workstations_id_seq') THEN
                EXECUTE format('ALTER SEQUENCE %I RENAME TO %I', 'seed_master_data_workstations_id_seq', 'apps_master_data_workstations_id_seq');
            END IF;
        END $$;
        ...
        -- 序列重命名：root_* → core_*
        -- ============================================
        -- 系统级表序列重命名（使用 DO 块进行条件检查和异常处理）
        DO $$
        DECLARE
            seq_pairs TEXT[][] := ARRAY[
                ['root_apis_id_seq', 'core_apis_id_seq'],
                ['root_applications_id_seq', 'core_applications_id_seq'],
                ['root_approval_instances_id_seq', 'core_approval_instances_id_seq'],
                ['root_approval_processes_id_seq', 'core_approval_processes_id_seq'],
                ['root_data_backups_id_seq', 'core_data_backups_id_seq'],
                ['root_data_sources_id_seq', 'core_data_sources_id_seq'],
                ['root_datasets_id_seq', 'core_datasets_id_seq'],
                ['root_departments_id_seq', 'core_departments_id_seq'],
                ['root_electronic_records_id_seq', 'core_electronic_records_id_seq'],
                ['root_files_id_seq', 'core_files_id_seq'],
                ['root_integration_configs_id_seq', 'core_integration_configs_id_seq'],
                ['root_login_logs_id_seq', 'core_login_logs_id_seq'],
                ['root_menus_id_seq', 'core_menus_id_seq'],
                ['root_message_configs_id_seq', 'core_message_configs_id_seq'],
                ['root_message_logs_id_seq', 'core_message_logs_id_seq'],
                ['root_message_templates_id_seq', 'core_message_templates_id_seq'],
                ['root_operation_logs_id_seq', 'core_operation_logs_id_seq'],
                ['root_positions_id_seq', 'core_positions_id_seq'],
                ['root_print_devices_id_seq', 'core_print_devices_id_seq'],
                ['root_print_templates_id_seq', 'core_print_templates_id_seq'],
                ['root_role_permissions_id_seq', 'core_role_permissions_id_seq'],
                ['root_saved_searches_id_seq', 'core_saved_searches_id_seq'],
                ['root_scheduled_tasks_id_seq', 'core_scheduled_tasks_id_seq'],
                ['root_scripts_id_seq', 'core_scripts_id_seq'],
                ['root_user_preferences_id_seq', 'core_user_preferences_id_seq'],
                ['root_user_roles_id_seq', 'core_user_roles_id_seq']
            ];
            pair TEXT[];
        BEGIN
            FOREACH pair SLICE 1 IN ARRAY seq_pairs
            LOOP
                BEGIN
                    IF EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'public' AND c.relname = pair[1] AND c.relkind = 'S')
                       AND NOT EXISTS (SELECT 1 FROM pg_class c JOIN pg_namespace n ON n.oid = c.relnamespace WHERE n.nspname = 'public' AND c.relname = pair[2] AND c.relkind = 'S') THEN
        ...
        DECLARE
            seq_pairs TEXT[][] := ARRAY[
                ['sys_code_rules_id_seq', 'core_code_rules_id_seq'],
                ['sys_code_sequences_id_seq', 'core_code_sequences_id_seq'],
                ['sys_custom_field_values_id_seq', 'core_custom_field_values_id_seq'],
                ['sys_custom_fields_id_seq', 'core_custom_fields_id_seq'],
                ['sys_data_dictionaries_id_seq', 'core_data_dictionaries_id_seq'],
                ['sys_dictionary_items_id_seq', 'core_dictionary_items_id_seq'],
                ['sys_invitation_codes_id_seq', 'core_invitation_codes_id_seq'],
                ['sys_languages_id_seq', 'core_languages_id_seq'],
                ['sys_site_settings_id_seq', 'core_site_settings_id_seq'],
                ['sys_system_parameters_id_seq', 'core_system_parameters_id_seq']
            ];
            pair TEXT[];
        BEGIN
            FOREACH pair SLICE 1 IN ARRAY seq_pairs
        ...
        DECLARE
            seq_pairs TEXT[][] := ARRAY[
                ['soil_packages_id_seq', 'infra_packages_id_seq'],
                ['soil_platform_superadmin_id_seq', 'infra_superadmin_id_seq']
            ];
            pair TEXT[];
        BEGIN
            FOREACH pair SLICE 1 IN ARRAY seq_pairs
        ...
        ALTER TABLE IF EXISTS "apps_master_data_customers" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_customers_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_defect_types" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_defect_types_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_holidays" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_holidays_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_material_groups" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_material_groups_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_materials" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_materials_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_operations" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_operations_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_process_routes" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_process_routes_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_production_lines" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_production_lines_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_products" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_products_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_skills" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_skills_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_sop_executions" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_sop_executions_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_sop" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_sop_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_storage_areas" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_storage_areas_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_storage_locations" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_storage_locations_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_suppliers" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_suppliers_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_warehouses" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_warehouses_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_workshops" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_workshops_id_seq'::regclass);
        ALTER TABLE IF EXISTS "apps_master_data_workstations" 
            ALTER COLUMN "id" SET DEFAULT nextval('apps_master_data_workstations_id_seq'::regclass);

        -- 系统级表（root_前缀）
        ALTER TABLE IF EXISTS "core_apis" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_apis_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_applications" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_applications_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_approval_instances" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_approval_instances_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_approval_processes" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_approval_processes_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_data_backups" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_data_backups_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_data_sources" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_data_sources_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_datasets" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_datasets_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_departments" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_departments_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_electronic_records" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_electronic_records_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_files" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_files_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_integration_configs" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_integration_configs_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_login_logs" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_login_logs_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_menus" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_menus_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_message_configs" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_message_configs_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_message_logs" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_message_logs_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_message_templates" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_message_templates_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_operation_logs" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_operation_logs_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_positions" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_positions_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_print_devices" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_print_devices_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_print_templates" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_print_templates_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_role_permissions" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_role_permissions_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_saved_searches" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_saved_searches_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_scheduled_tasks" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_scheduled_tasks_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_scripts" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_scripts_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_user_preferences" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_user_preferences_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_user_roles" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_user_roles_id_seq'::regclass);

        -- 系统级表（sys_前缀）
        ALTER TABLE IF EXISTS "core_code_rules" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_code_rules_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_code_sequences" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_code_sequences_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_custom_field_values" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_custom_field_values_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_custom_fields" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_custom_fields_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_data_dictionaries" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_data_dictionaries_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_dictionary_items" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_dictionary_items_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_invitation_codes" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_invitation_codes_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_languages" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_languages_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_site_settings" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_site_settings_id_seq'::regclass);
        ALTER TABLE IF EXISTS "core_system_parameters" 
            ALTER COLUMN "id" SET DEFAULT nextval('core_system_parameters_id_seq'::regclass);

        -- 平台级表
        ALTER TABLE IF EXISTS "infra_packages" 
            ALTER COLUMN "id" SET DEFAULT nextval('infra_packages_id_seq'::regclass);
        ALTER TABLE IF EXISTS "infra_superadmin" 
            ALTER COLUMN "id" SET DEFAULT nextval('infra_superadmin_id_seq'::regclass);
    """
...
async def downgrade(db: BaseDBAsyncClient) -> str:
    ...
    警告：此操作会恢复序列的旧名称，可能导致表定义与序列名称不匹配。
    建议在生产环境中谨慎使用。
    """
    return """
        -- ============================================
        -- 恢复序列原始名称：apps_master_data_* → seed_*
        -- ============================================
        ALTER SEQUENCE IF EXISTS "apps_master_data_bom_id_seq" RENAME TO "seed_master_data_bom_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_customers_id_seq" RENAME TO "seed_master_data_customers_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_defect_types_id_seq" RENAME TO "seed_master_data_defect_types_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_holidays_id_seq" RENAME TO "seed_master_data_holidays_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_material_groups_id_seq" RENAME TO "seed_master_data_material_groups_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_materials_id_seq" RENAME TO "seed_master_data_materials_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_operations_id_seq" RENAME TO "seed_master_data_operations_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_process_routes_id_seq" RENAME TO "seed_master_data_process_routes_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_production_lines_id_seq" RENAME TO "seed_master_data_production_lines_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_products_id_seq" RENAME TO "seed_master_data_products_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_skills_id_seq" RENAME TO "seed_master_data_skills_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_sop_executions_id_seq" RENAME TO "seed_master_data_sop_executions_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_sop_id_seq" RENAME TO "seed_master_data_sop_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_storage_areas_id_seq" RENAME TO "seed_master_data_storage_areas_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_storage_locations_id_seq" RENAME TO "seed_master_data_storage_locations_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_suppliers_id_seq" RENAME TO "seed_master_data_suppliers_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_warehouses_id_seq" RENAME TO "seed_master_data_warehouses_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_workshops_id_seq" RENAME TO "seed_master_data_workshops_id_seq";
        ALTER SEQUENCE IF EXISTS "apps_master_data_workstations_id_seq" RENAME TO "seed_master_data_workstations_id_seq";

        -- ============================================
        -- 恢复序列原始名称：core_* → root_*
        -- ============================================
        ALTER SEQUENCE IF EXISTS "core_apis_id_seq" RENAME TO "root_apis_id_seq";
        ALTER SEQUENCE IF EXISTS "core_applications_id_seq" RENAME TO "root_applications_id_seq";
        ALTER SEQUENCE IF EXISTS "core_approval_instances_id_seq" RENAME TO "root_approval_instances_id_seq";
        ALTER SEQUENCE IF EXISTS "core_approval_processes_id_seq" RENAME TO "root_approval_processes_id_seq";
        ALTER SEQUENCE IF EXISTS "core_data_backups_id_seq" RENAME TO "root_data_backups_id_seq";
        ALTER SEQUENCE IF EXISTS "core_data_sources_id_seq" RENAME TO "root_data_sources_id_seq";
        ALTER SEQUENCE IF EXISTS "core_datasets_id_seq" RENAME TO "root_datasets_id_seq";
        ALTER SEQUENCE IF EXISTS "core_departments_id_seq" RENAME TO "root_departments_id_seq";
        ALTER SEQUENCE IF EXISTS "core_electronic_records_id_seq" RENAME TO "root_electronic_records_id_seq";
        ALTER SEQUENCE IF EXISTS "core_files_id_seq" RENAME TO "root_files_id_seq";
        ALTER SEQUENCE IF EXISTS "core_integration_configs_id_seq" RENAME TO "root_integration_configs_id_seq";
        ALTER SEQUENCE IF EXISTS "core_login_logs_id_seq" RENAME TO "root_login_logs_id_seq";
        ALTER SEQUENCE IF EXISTS "core_menus_id_seq" RENAME TO "root_menus_id_seq";
        ALTER SEQUENCE IF EXISTS "core_message_configs_id_seq" RENAME TO "root_message_configs_id_seq";
        ALTER SEQUENCE IF EXISTS "core_message_logs_id_seq" RENAME TO "root_message_logs_id_seq";
        ALTER SEQUENCE IF EXISTS "core_message_templates_id_seq" RENAME TO "root_message_templates_id_seq";
        ALTER SEQUENCE IF EXISTS "core_operation_logs_id_seq" RENAME TO "root_operation_logs_id_seq";
        ALTER SEQUENCE IF EXISTS "core_positions_id_seq" RENAME TO "root_positions_id_seq";
        ALTER SEQUENCE IF EXISTS "core_print_devices_id_seq" RENAME TO "root_print_devices_id_seq";
        ALTER SEQUENCE IF EXISTS "core_print_templates_id_seq" RENAME TO "root_print_templates_id_seq";
        ALTER SEQUENCE IF EXISTS "core_role_permissions_id_seq" RENAME TO "root_role_permissions_id_seq";
        ALTER SEQUENCE IF EXISTS "core_saved_searches_id_seq" RENAME TO "root_saved_searches_id_seq";
        ALTER SEQUENCE IF EXISTS "core_scheduled_tasks_id_seq" RENAME TO "root_scheduled_tasks_id_seq";
        ALTER SEQUENCE IF EXISTS "core_scripts_id_seq" RENAME TO "root_scripts_id_seq";
        ALTER SEQUENCE IF EXISTS "core_user_preferences_id_seq" RENAME TO "root_user_preferences_id_seq";
        ALTER SEQUENCE IF EXISTS "core_user_roles_id_seq" RENAME TO "root_user_roles_id_seq";

        -- ============================================
        -- 恢复序列原始名称：core_* → sys_*
        -- ============================================
        ALTER SEQUENCE IF EXISTS "core_code_rules_id_seq" RENAME TO "sys_code_rules_id_seq";
        ALTER SEQUENCE IF EXISTS "core_code_sequences_id_seq" RENAME TO "sys_code_sequences_id_seq";
        ALTER SEQUENCE IF EXISTS "core_custom_field_values_id_seq" RENAME TO "sys_custom_field_values_id_seq";
        ALTER SEQUENCE IF EXISTS "core_custom_fields_id_seq" RENAME TO "sys_custom_fields_id_seq";
        ALTER SEQUENCE IF EXISTS "core_data_dictionaries_id_seq" RENAME TO "sys_data_dictionaries_id_seq";
        ALTER SEQUENCE IF EXISTS "core_dictionary_items_id_seq" RENAME TO "sys_dictionary_items_id_seq";
        ALTER SEQUENCE IF EXISTS "core_invitation_codes_id_seq" RENAME TO "sys_invitation_codes_id_seq";
        ALTER SEQUENCE IF EXISTS "core_languages_id_seq" RENAME TO "sys_languages_id_seq";
        ALTER SEQUENCE IF EXISTS "core_site_settings_id_seq" RENAME TO "sys_site_settings_id_seq";
        ALTER SEQUENCE IF EXISTS "core_system_parameters_id_seq" RENAME TO "sys_system_parameters_id_seq";

        -- ============================================
        -- 恢复序列原始名称：infra_* → soil_*
        -- ============================================
        ALTER SEQUENCE IF EXISTS "infra_packages_id_seq" RENAME TO "soil_packages_id_seq";
        ALTER SEQUENCE IF EXISTS "infra_superadmin_id_seq" RENAME TO "soil_platform_superadmin_id_seq";

        -- ============================================
        -- 恢复表定义中的序列引用
        -- ============================================
        -- 注意：降级时需要恢复表定义中的序列引用，但为了简化，这里只恢复序列名称
        -- 表定义的序列引用会在下次迁移时自动更新
    """
