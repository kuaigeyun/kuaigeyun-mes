...
为数据库表添加缺失的主键约束，确保数据完整性和外键约束的正常工作。

变更内容：
- 系统级 (core_*) 表的主键约束
- 应用级主数据 (apps_master_data_*) 表的主键约束
- 平台级 (infra_*) 表的主键约束
- 使用幂等性检查，如果主键已存在则跳过
    ...
    return """
        ...
        BEGIN
            IF NOT EXISTS (
                ...
                WHERE conrelid = 'core_users'::regclass
                AND contype = 'p'
            ) THEN
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_roles'::regclass
                AND contype = 'p'
            ) THEN
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_permissions'::regclass
                AND contype = 'p'
            ) THEN
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'infra_tenants'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "infra_tenants" 
                ADD CONSTRAINT "core_tenants_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        
        -- core_tenant_configs 表
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'infra_tenant_configs'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "infra_tenant_configs" 
                ADD CONSTRAINT "core_tenant_configs_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_departments'::regclass
                AND contype = 'p'
            ) THEN
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_positions'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_positions" 
                ADD CONSTRAINT "core_positions_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_menus'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_menus" 
                ADD CONSTRAINT "core_menus_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_data_sources'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_data_sources" 
                ADD CONSTRAINT "core_data_sources_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_datasets'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_datasets" 
                ADD CONSTRAINT "core_datasets_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        
        -- core_approval_processes 表
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_approval_processes'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_approval_processes" 
                ADD CONSTRAINT "core_approval_processes_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_approval_instances'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_approval_instances" 
                ADD CONSTRAINT "core_approval_instances_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_approval_histories'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_approval_histories" 
                ADD CONSTRAINT "core_approval_histories_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        
        -- core_data_dictionaries 表
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_data_dictionaries'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_data_dictionaries" 
                ADD CONSTRAINT "core_data_dictionaries_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_dictionary_items'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_dictionary_items" 
                ADD CONSTRAINT "core_dictionary_items_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_code_rules'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_code_rules" 
                ADD CONSTRAINT "core_code_rules_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_code_sequences'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_code_sequences" 
                ADD CONSTRAINT "core_code_sequences_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        
        -- core_custom_fields 表
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_custom_fields'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_custom_fields" 
                ADD CONSTRAINT "core_custom_fields_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_custom_field_values'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_custom_field_values" 
                ADD CONSTRAINT "core_custom_field_values_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_user_roles'::regclass
                AND contype = 'p'
            ) THEN
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_role_permissions'::regclass
                AND contype = 'p'
            ) THEN
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_saved_searches'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_saved_searches" 
                ADD CONSTRAINT "core_saved_searches_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'core_user_preferences'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "core_user_preferences" 
                ADD CONSTRAINT "core_user_preferences_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        -- apps_master_data_material_groups 表
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_material_groups'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_material_groups" 
                ADD CONSTRAINT "apps_master_data_material_groups_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_materials'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_materials" 
                ADD CONSTRAINT "apps_master_data_materials_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_bom'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_bom" 
                ADD CONSTRAINT "apps_master_data_bom_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_warehouses'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_warehouses" 
                ADD CONSTRAINT "apps_master_data_warehouses_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_storage_areas'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_storage_areas" 
                ADD CONSTRAINT "apps_master_data_storage_areas_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_storage_locations'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_storage_locations" 
                ADD CONSTRAINT "apps_master_data_storage_locations_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        
        -- apps_master_data_workshops 表
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_workshops'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_workshops" 
                ADD CONSTRAINT "apps_master_data_workshops_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_production_lines'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_production_lines" 
                ADD CONSTRAINT "apps_master_data_production_lines_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_workstations'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_workstations" 
                ADD CONSTRAINT "apps_master_data_workstations_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        
        -- apps_master_data_sop 表
        DO $$
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_sop'::regclass
                AND contype = 'p'
            ) THEN
                ALTER TABLE "apps_master_data_sop" 
                ADD CONSTRAINT "apps_master_data_sop_pkey" PRIMARY KEY ("id");
            END IF;
        END $$;
        ...
        BEGIN
            IF NOT EXISTS (
                SELECT 1 FROM pg_constraint 
                WHERE conrelid = 'apps_master_data_sop_executions'::regclass
                AND contype = 'p'
            ) THEN
...
async def downgrade(db: BaseDBAsyncClient) -> str:
    ...
    return """
        ...
        ALTER TABLE IF EXISTS "apps_master_data_sop_executions" 
            DROP CONSTRAINT IF EXISTS "apps_master_data_sop_executions_pkey";
        
        ALTER TABLE IF EXISTS "apps_master_data_sop" 
            DROP CONSTRAINT IF EXISTS "apps_master_data_sop_pkey";
        
        ALTER TABLE IF EXISTS "apps_master_data_workstations" 
            DROP CONSTRAINT IF EXISTS "apps_master_data_workstations_pkey";
        
        ALTER TABLE IF EXISTS "apps_master_data_production_lines" 
            DROP CONSTRAINT IF EXISTS "apps_master_data_production_lines_pkey";
        
        ALTER TABLE IF EXISTS "apps_master_data_workshops" 
            DROP CONSTRAINT IF EXISTS "apps_master_data_workshops_pkey";
        
        ALTER TABLE IF EXISTS "apps_master_data_storage_locations" 
            DROP CONSTRAINT IF EXISTS "apps_master_data_storage_locations_pkey";
        
        ALTER TABLE IF EXISTS "apps_master_data_storage_areas" 
            DROP CONSTRAINT IF EXISTS "apps_master_data_storage_areas_pkey";
        
        ALTER TABLE IF EXISTS "apps_master_data_warehouses" 
            DROP CONSTRAINT IF EXISTS "apps_master_data_warehouses_pkey";
        ...
        ALTER TABLE IF EXISTS "core_user_preferences" 
            DROP CONSTRAINT IF EXISTS "core_user_preferences_pkey";
        
        ALTER TABLE IF EXISTS "core_saved_searches" 
            DROP CONSTRAINT IF EXISTS "core_saved_searches_pkey";
        
        ALTER TABLE IF EXISTS "core_role_permissions" 
            DROP CONSTRAINT IF EXISTS "core_role_permissions_pkey";
        
        ALTER TABLE IF EXISTS "core_user_roles" 
            DROP CONSTRAINT IF EXISTS "core_user_roles_pkey";
        
        ALTER TABLE IF EXISTS "core_custom_field_values" 
            DROP CONSTRAINT IF EXISTS "core_custom_field_values_pkey";
        
        ALTER TABLE IF EXISTS "core_custom_fields" 
            DROP CONSTRAINT IF EXISTS "core_custom_fields_pkey";
        
        ALTER TABLE IF EXISTS "core_code_sequences" 
            DROP CONSTRAINT IF EXISTS "core_code_sequences_pkey";
        
        ALTER TABLE IF EXISTS "core_code_rules" 
            DROP CONSTRAINT IF EXISTS "core_code_rules_pkey";
        
        ALTER TABLE IF EXISTS "core_dictionary_items" 
            DROP CONSTRAINT IF EXISTS "core_dictionary_items_pkey";
        
        ALTER TABLE IF EXISTS "core_data_dictionaries" 
            DROP CONSTRAINT IF EXISTS "core_data_dictionaries_pkey";
        
        ALTER TABLE IF EXISTS "core_approval_histories" 
            DROP CONSTRAINT IF EXISTS "core_approval_histories_pkey";
        
        ALTER TABLE IF EXISTS "core_approval_instances" 
            DROP CONSTRAINT IF EXISTS "core_approval_instances_pkey";
        
        ALTER TABLE IF EXISTS "core_approval_processes" 
            DROP CONSTRAINT IF EXISTS "core_approval_processes_pkey";
        
        ALTER TABLE IF EXISTS "core_datasets" 
            DROP CONSTRAINT IF EXISTS "core_datasets_pkey";
        ...
        ALTER TABLE IF EXISTS "core_positions" 
            DROP CONSTRAINT IF EXISTS "core_positions_pkey";
        
        ALTER TABLE IF EXISTS "core_departments" 
            DROP CONSTRAINT IF EXISTS "core_departments_pkey";
        
        ALTER TABLE IF EXISTS "infra_tenant_configs" 
            DROP CONSTRAINT IF EXISTS "core_tenant_configs_pkey";
        
        ALTER TABLE IF EXISTS "infra_tenants" 
            DROP CONSTRAINT IF EXISTS "core_tenants_pkey";
        
        ALTER TABLE IF EXISTS "core_permissions" 
            DROP CONSTRAINT IF EXISTS "core_permissions_pkey";
        
        ALTER TABLE IF EXISTS "core_roles" 
            DROP CONSTRAINT IF EXISTS "core_roles_pkey";
        
        ALTER TABLE IF EXISTS "core_users" 
            DROP CONSTRAINT IF EXISTS "core_users_pkey";
    """
