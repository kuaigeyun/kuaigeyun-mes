...
为数据库表添加缺失的复合索引，优化常用组合查询的性能。

变更内容：
- 系统级 (core_*) 表的复合索引
- 应用级主数据 (apps_master_data_*) 表的复合索引
- 使用幂等性检查，如果索引已存在则跳过
    ...
    return """
        ...
        BEGIN
            ...
            IF NOT EXISTS (
                ...
                AND tablename = 'core_message_logs' 
                AND indexname = 'idx_core_message_logs_tenant_id_status'
            ) THEN
                CREATE INDEX "idx_core_message_logs_tenant_id_status" 
                ON "core_message_logs" ("tenant_id", "status");
            END IF;
            ...
            IF NOT EXISTS (
                ...
                WHERE schemaname = 'public' 
                AND tablename = 'core_message_logs' 
                AND indexname = 'idx_core_message_logs_tenant_id_type_status'
            ) THEN
                CREATE INDEX "idx_core_message_logs_tenant_id_type_status" 
                ON "core_message_logs" ("tenant_id", "type", "status");
            END IF;
            
            -- tenant_id + recipient + status (按组织+收件人+状态查询)
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes 
                WHERE schemaname = 'public' 
                AND tablename = 'core_message_logs' 
                AND indexname = 'idx_core_message_logs_tenant_id_recipient_status'
            ) THEN
                CREATE INDEX "idx_core_message_logs_tenant_id_recipient_status" 
                ON "core_message_logs" ("tenant_id", "recipient", "status");
            END IF;
            
            -- tenant_id + created_at (按组织+创建时间查询)
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes 
                WHERE schemaname = 'public' 
                AND tablename = 'core_message_logs' 
                AND indexname = 'idx_core_message_logs_tenant_id_created_at'
            ) THEN
                CREATE INDEX "idx_core_message_logs_tenant_id_created_at" 
                ON "core_message_logs" ("tenant_id", "created_at");
            END IF;
        END $$;
        ...
        BEGIN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes 
                WHERE schemaname = 'public' 
                AND tablename = 'core_login_logs' 
                AND indexname = 'idx_core_login_logs_tenant_id_user_id'
            ) THEN
                CREATE INDEX "idx_core_login_logs_tenant_id_user_id" 
                ON "core_login_logs" ("tenant_id", "user_id");
            END IF;
            ...
            IF NOT EXISTS (
                ...
                WHERE schemaname = 'public' 
                AND tablename = 'core_login_logs' 
                AND indexname = 'idx_core_login_logs_tenant_id_login_status'
            ) THEN
                CREATE INDEX "idx_core_login_logs_tenant_id_login_status" 
                ON "core_login_logs" ("tenant_id", "login_status");
            END IF;
            ...
            IF NOT EXISTS (
                ...
                WHERE schemaname = 'public' 
                AND tablename = 'core_login_logs' 
                AND indexname = 'idx_core_login_logs_tenant_id_created_at'
            ) THEN
                CREATE INDEX "idx_core_login_logs_tenant_id_created_at" 
                ON "core_login_logs" ("tenant_id", "created_at");
            END IF;
            ...
            IF NOT EXISTS (
                ...
                WHERE schemaname = 'public' 
                AND tablename = 'core_login_logs' 
                AND indexname = 'idx_core_login_logs_tenant_id_user_id_created_at'
            ) THEN
                CREATE INDEX "idx_core_login_logs_tenant_id_user_id_created_at" 
                ON "core_login_logs" ("tenant_id", "user_id", "created_at");
            END IF;
        END $$;
        ...
        BEGIN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes 
                WHERE schemaname = 'public' 
                AND tablename = 'core_approval_instances' 
                AND indexname = 'idx_core_approval_instances_tenant_id_status'
            ) THEN
                CREATE INDEX "idx_core_approval_instances_tenant_id_status" 
                ON "core_approval_instances" ("tenant_id", "status");
            END IF;
            
            -- tenant_id + current_approver_id + status (按组织+审批人+状态查询)
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes 
                WHERE schemaname = 'public' 
                AND tablename = 'core_approval_instances' 
                AND indexname = 'idx_core_approval_instances_tenant_id_current_approver_id_status'
            ) THEN
                CREATE INDEX "idx_core_approval_instances_tenant_id_current_approver_id_status" 
                ON "core_approval_instances" ("tenant_id", "current_approver_id", "status");
            END IF;
            
            -- tenant_id + submitter_id (按组织+提交人查询)
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes 
                WHERE schemaname = 'public' 
                AND tablename = 'core_approval_instances' 
                AND indexname = 'idx_core_approval_instances_tenant_id_submitter_id'
            ) THEN
                CREATE INDEX "idx_core_approval_instances_tenant_id_submitter_id" 
                ON "core_approval_instances" ("tenant_id", "submitter_id");
            END IF;
            ...
            IF NOT EXISTS (
                ...
                WHERE schemaname = 'public' 
                AND tablename = 'core_approval_instances' 
                AND indexname = 'idx_core_approval_instances_tenant_id_created_at'
            ) THEN
                CREATE INDEX "idx_core_approval_instances_tenant_id_created_at" 
                ON "core_approval_instances" ("tenant_id", "created_at");
            END IF;
        END $$;
        
        -- core_operation_logs 表
        DO $$
        BEGIN
            -- tenant_id + user_id (按组织+用户查询)
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes 
                WHERE schemaname = 'public' 
                AND tablename = 'core_operation_logs' 
                AND indexname = 'idx_core_operation_logs_tenant_id_user_id'
            ) THEN
                CREATE INDEX "idx_core_operation_logs_tenant_id_user_id" 
                ON "core_operation_logs" ("tenant_id", "user_id");
            END IF;
            ...
            IF NOT EXISTS (
                ...
                WHERE schemaname = 'public' 
                AND tablename = 'core_operation_logs' 
                AND indexname = 'idx_core_operation_logs_tenant_id_created_at'
            ) THEN
                CREATE INDEX "idx_core_operation_logs_tenant_id_created_at" 
                ON "core_operation_logs" ("tenant_id", "created_at");
            END IF;
            ...
            IF NOT EXISTS (
                ...
                WHERE schemaname = 'public' 
                AND tablename = 'core_operation_logs' 
                AND indexname = 'idx_core_operation_logs_tenant_id_operation_module'
            ) THEN
                CREATE INDEX "idx_core_operation_logs_tenant_id_operation_module" 
                ON "core_operation_logs" ("tenant_id", "operation_module");
            END IF;
            ...
            IF NOT EXISTS (
                ...
                WHERE schemaname = 'public' 
                AND tablename = 'core_operation_logs' 
                AND indexname = 'idx_core_operation_logs_tenant_id_user_id_created_at'
            ) THEN
                CREATE INDEX "idx_core_operation_logs_tenant_id_user_id_created_at" 
                ON "core_operation_logs" ("tenant_id", "user_id", "created_at");
            END IF;
        END $$;
        ...
        BEGIN
            ...
            IF NOT EXISTS (
                SELECT 1 FROM pg_indexes 
                WHERE schemaname = 'public' 
                AND tablename = 'core_saved_searches' 
                AND indexname = 'idx_core_saved_searches_tenant_id_user_id_page_path'
            ) THEN
                CREATE INDEX "idx_core_saved_searches_tenant_id_user_id_page_path" 
                ON "core_saved_searches" ("tenant_id", "user_id", "page_path");
            END IF;
        END $$;
        ...
        -- apps_master_data_sop_executions 表的复合索引已在 0_init_schema.py 中定义，这里不再重复添加
        -- 但为了完整性，我们检查一下是否已存在
        -- 注意：如果索引已存在，DO 块会跳过创建
    """
...
async def downgrade(db: BaseDBAsyncClient) -> str:
    ...
    return """
        ...
        -- 删除应用级主数据表的复合索引
        -- ============================================
        -- apps_master_data_sop_executions 表的复合索引保留（已在 0_init_schema.py 中定义）
        
        -- ============================================
        -- 删除系统级表的复合索引
        -- ============================================
        DROP INDEX IF EXISTS "idx_core_saved_searches_tenant_id_user_id_page_path";
        
        DROP INDEX IF EXISTS "idx_core_operation_logs_tenant_id_user_id_created_at";
        DROP INDEX IF EXISTS "idx_core_operation_logs_tenant_id_operation_module";
        DROP INDEX IF EXISTS "idx_core_operation_logs_tenant_id_created_at";
        DROP INDEX IF EXISTS "idx_core_operation_logs_tenant_id_user_id";
        
        DROP INDEX IF EXISTS "idx_core_approval_instances_tenant_id_created_at";
        DROP INDEX IF EXISTS "idx_core_approval_instances_tenant_id_submitter_id";
        DROP INDEX IF EXISTS "idx_core_approval_instances_tenant_id_current_approver_id_status";
        DROP INDEX IF EXISTS "idx_core_approval_instances_tenant_id_status";
        
        DROP INDEX IF EXISTS "idx_core_login_logs_tenant_id_user_id_created_at";
        DROP INDEX IF EXISTS "idx_core_login_logs_tenant_id_created_at";
        DROP INDEX IF EXISTS "idx_core_login_logs_tenant_id_login_status";
        DROP INDEX IF EXISTS "idx_core_login_logs_tenant_id_user_id";
        
        DROP INDEX IF EXISTS "idx_core_message_logs_tenant_id_created_at";
        DROP INDEX IF EXISTS "idx_core_message_logs_tenant_id_recipient_status";
        DROP INDEX IF EXISTS "idx_core_message_logs_tenant_id_type_status";
        DROP INDEX IF EXISTS "idx_core_message_logs_tenant_id_status";
    """
