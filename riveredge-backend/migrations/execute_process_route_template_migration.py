...
async def execute_migration():
    """执行迁移"""
    migration_file = Path(__file__).parent / "models" / "29_20260116000004_create_process_route_template.py"
    ...
    try:
        async with conn.transaction():
            ...
            columns = await conn.fetch("""
                ...
                WHERE table_name = 'apps_master_data_process_route_templates'
                ORDER BY ordinal_position
            """)
            if columns:
                print(f"  ✓ 表已创建，共 {len(columns)} 个字段：")
                for col in columns:
            ...
            indexes = await conn.fetch("""
                SELECT indexname
                FROM pg_indexes
                WHERE tablename = 'apps_master_data_process_route_templates'
                ORDER BY indexname
            """)
            if indexes:
                print(f"  ✓ 索引已创建，共 {len(indexes)} 个：")
                for idx in indexes:
