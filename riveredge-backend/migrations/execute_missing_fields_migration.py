...
直接执行 51_20260120000007_add_missing_fields_to_cost_rules.py 中的 SQL 语句。
...
async def execute_migration():
    """执行迁移"""
    # 读取迁移文件
    migration_file = Path(__file__).parent / "models" / "51_20260120000007_add_missing_fields_to_cost_rules.py"
    ...
    try:
        ...
        # 验证字段是否已添加
        print("\n🔍 验证字段是否已添加...")
        fields_to_check = ['rule_type', 'calculation_method', 'calculation_formula', 'is_active']
        for field in fields_to_check:
            ...
            if field_exists:
                print(f"✅ 字段 {field} 已成功添加到表中")
            else:
                print(f"⚠️  警告：字段 {field} 可能未成功添加，请检查")
