...
**RiverEdge SaaS 多组织框架**采用**常规 B 端项目设计**，遵循企业级应用的最佳实践：

- **platform** (平台): 平台级后端，提供平台运营和管理功能
- **core** (核心): 系统级后端，提供组织内系统管理功能
- **app** (应用): 应用入口层，整合平台级和系统级功能
- **routes** (路由): 前端路由配置，定义应用路由规则
- **components** (组件): 前端公共组件，可复用的 UI 组件
- **services** (服务): 前端 API 服务层，封装后端 API 调用
- **seed** (插件): 应用插件，可扩展的业务功能模块

所有命名都遵循常规 B 端项目规范，清晰明确，易于理解和维护。
...
### 1. riveredge-backend (后端核心系统) - 平台级 + 系统级
...
- **Web 框架**: FastAPI 0.104.1 ⭐ **FIR 核心** (高性能异步 Web 框架)
- **ASGI 服务器**: Uvicorn 0.30.0 (ASGI 服务器，FastAPI 官方推荐)
- **任务调度**: Inngest ⭐ **FIR 核心** (现代化任务调度系统，处理异步任务和工作流)
...
### 2. riveredge-frontend (前端应用) - 平台级 + 系统级
...
5. 注册插件的数据库模型
6. 执行插件的数据库迁移
7. 注册插件的 Inngest 函数（如果存在）⭐ FIR 核心
...
**核心技术栈**:
- **编程语言**: Python 3.11 LTS (长期支持版本，推荐3.11.9)
- **Web 框架**: FastAPI 0.104.1 ⭐ **FIR 核心** (稳定兼容版本，与 Tortoise ORM 0.20.1 完美兼容)
- **ASGI 服务器**: Uvicorn 0.30.0 (ASGI 服务器，FastAPI 官方推荐)
- **任务调度**: Inngest ⭐ **FIR 核心** (现代化任务调度系统，处理异步任务和工作流)
- **ORM 框架**: Tortoise ORM 0.20.1 (稳定版，与FastAPI 0.104.1完美兼容)
- **数据库**: PostgreSQL 15+ (推荐15.6)
...
1. ✅ **清晰的模块划分**: 职责明确，易于理解
   - **platform**: 平台级功能模块（平台运营和管理）
   - **core**: 系统级功能模块（组织内系统管理）
   - **app**: 应用入口层（整合所有功能）
   - **routes**: 前端路由配置
   - **components**: 前端公共组件
   - **services**: 前端 API 服务层
   - **seed**: 应用插件集合（可扩展的业务功能）
...
4. ✅ **技术栈现代化**: 采用最新稳定版本，企业级最佳实践
   - 后端: Python 3.11 LTS + FastAPI 0.104.1 + Inngest + Tortoise ORM 0.20.1 + PostgreSQL 15+
...
**最后更新**：2025-01-04（采用 FIR 技术栈重构：FastAPI + Inngest + React）
