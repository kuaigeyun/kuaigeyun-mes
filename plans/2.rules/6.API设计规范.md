# RiverEdge SaaS 多组织框架 - API 设计规范

## 📋 概述

本文档定义 **RiverEdge SaaS 多组织框架 (RiverEdge SaaS Multi-tenant Framework)** 的**统一 API 设计规范**，确保前后端协作顺畅，接口风格一致，提升开发效率和代码可维护性。
...
4. **多组织隔离**：所有 API 自动处理组织隔离，无需手动传递 `tenant_id`
...
- ✅ `/api/v1/tenants/{tenant_id}/users` - 获取组织的用户列表
...
**超级管理员 API**（需要特殊权限）：
```
GET /api/v1/admin/tenants - 获取所有组织列表
POST /api/v1/admin/tenants - 创建组织
PUT /api/v1/admin/tenants/{tenant_id} - 更新组织
DELETE /api/v1/admin/tenants/{tenant_id} - 删除组织
```

**组织注册 API**（公开 API）：
```
POST /api/v1/auth/register - 组织注册
Body: {
  "tenant_name": "示例公司",
  "username": "admin",
  "email": "admin@example.com",
...
```python
# ✅ 正确示例
router = APIRouter(prefix="/auth", tags=["Authentication"])  # Tags: 英文
...
### 前端实现（TypeScript + TanStack Query）
...
- [ ] 排序参数使用 `sort` 和 `order`
- [ ] 多组织隔离自动处理（不手动传递 `tenant_id`）
- [ ] JWT Token 自动包含组织信息
- [ ] 错误响应格式统一
- [ ] API 文档自动生成（FastAPI OpenAPI）
- [ ] API Tags 使用英文（如 `Authentication`, `Users`, `Roles`）
- [ ] API 描述使用中文（函数文档字符串）
