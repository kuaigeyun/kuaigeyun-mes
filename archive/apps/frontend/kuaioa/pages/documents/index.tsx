...
import { documentApi } from '../../services/process';
import type { Document, DocumentCreate, DocumentUpdate } from '../../types/process';
...
const DocumentPage: React.FC = () => {
  ...
  const handleEdit = async (record: Document) => {
    try {
      ...
      setModalVisible(true);
      const data = await documentApi.get(record.uuid);
  ...
  const handleDelete = async (record: Document) => {
    try {
      await documentApi.delete(record.uuid);
  ...
  const handleSubmit = async (values: DocumentCreate | DocumentUpdate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await documentApi.update(currentUuid, values as DocumentUpdate);
        messageApi.success('更新成功');
      } else {
        await documentApi.create(values as DocumentCreate);
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<Document>
        ...
        request={async (params) => {
          ...
          const data = await documentApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑文档' : '新建文档'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="documentNo" label="文档编号" rules={[{ required: true }]} disabled={isEdit} />
