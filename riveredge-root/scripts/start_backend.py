...
# ⚠️ 重要：Windows 上使用 multiprocessing（uvicorn reload）时必须使用 if __name__ == '__main__' 保护
if __name__ == '__main__':
    try:
        ...
        # ⚠️ 重要：启用 reload 时必须使用字符串形式的导入路径，不能直接传递 app 对象
        if enable_reload:
            # 使用字符串形式，让 uvicorn 自动导入（支持热重载）
            uvicorn.run(
                "app.main:app",  # 字符串形式的导入路径
                host="0.0.0.0",
                port=port,
