...
const SuperAdminTenantList: React.FC = () => {
  ...
  const loadTenantDetail = async (tenantId: number) => {
    ...
    try {
      // 使用超级管理员接口获取组织详情
      const data = await apiRequest<Tenant>(`/superadmin/tenants/${tenantId}`, {
        method: 'GET',
      });
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (isEdit && currentTenantId) {
        ...
        // 使用超级管理员接口更新组织
        await apiRequest<Tenant>(`/superadmin/tenants/${currentTenantId}`, {
          method: 'PUT',
          data: updateData,
        });
        ...
        // 使用超级管理员接口创建组织
        await apiRequest<Tenant>('/superadmin/tenants', {
          method: 'POST',
          data: createData,
        });
            ...
            {tenantDetail.status === TenantStatus.INACTIVE && (
              <>
                <Popconfirm
                  ...
                  onConfirm={async () => {
                    try {
                      await apiRequest(`/superadmin/tenants/${currentTenantId}/approve`, {
                        method: 'POST',
                      });
                      message.success('审核通过成功');
                ...
                <Popconfirm
                  ...
                  onConfirm={async () => {
                    try {
                      await apiRequest(`/superadmin/tenants/${currentTenantId}/reject`, {
                        method: 'POST',
                      });
                      message.success('审核拒绝成功');
                      loadTenantDetail(currentTenantId!);
