...
const loadLuckysheet = async (): Promise<any> => {
  ...
  try {
    ...
    if (!luckysheetInstance || typeof luckysheetInstance.create !== 'function') {
      console.error('Luckysheet 对象没有 create 方法');
 ...
 * Luckysheet 导入弹窗组件属性
 */
export interface LuckysheetImportModalProps {
  /**
   * 弹窗是否可见
   */
  visible: boolean;
 ...
 * Luckysheet 导入弹窗组件
 */
export const LuckysheetImportModal: React.FC<LuckysheetImportModalProps> = ({
  visible,
  onCancel,
  onConfirm,
  title = '导入数据',
    ...
    if (visible) {
      ...
      const initLuckysheet = async () => {
        ...
        try {
          ...
          setLoading(false);
          message.error('表格加载失败：' + (error.message || '未知错误，请查看控制台'));
        }
      };
...
export default LuckysheetImportModal;
