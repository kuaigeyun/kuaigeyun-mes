...
export const QuerySearchModal: React.FC<QuerySearchModalProps> = ({
  ...
  // ⭐ 修复：使用 useMemo 稳定 savedSearches 引用，避免无限循环
  const savedSearches = useMemo(() => savedSearchesData?.items || [], [savedSearchesData?.items]);
  
  // 分离个人和共享搜索条件
  // ⭐ 修复：使用 useMemo 直接计算，避免 useState + useEffect 导致的无限循环
  const { personalSearches, sharedSearches } = useMemo(() => {
    ...
    return {
      personalSearches: orderedPersonal,
      sharedSearches: orderedShared,
    };
  }, [savedSearches, pagePath]);
  
  // ⭐ 修复：使用 useState 存储排序后的列表，用于拖拽排序
  const [personalSearchesState, setPersonalSearchesState] = useState<SavedSearch[]>(personalSearches);
  const [sharedSearchesState, setSharedSearchesState] = useState<SavedSearch[]>(sharedSearches);
  
  // ⭐ 修复：当 savedSearches 变化时，同步更新状态
  useEffect(() => {
    setPersonalSearchesState(personalSearches);
    setSharedSearchesState(sharedSearches);
  }, [personalSearches, sharedSearches]);
  ...
  const handlePersonalDragEnd = useCallback((event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      setPersonalSearchesState((items) => {
        const oldIndex = items.findIndex((item) => item.id === active.id);
        const newIndex = items.findIndex((item) => item.id === over.id);
        const newItems = arrayMove(items, oldIndex, newIndex);
  ...
  const updateSavedSearchMutation = useMutation({
    mutationFn: ({ uuid, data }: { uuid: string; data: any }) => updateSavedSearch(uuid, data),
    onSuccess: () => {
    ...
    <>
      ...
      <div style={{ display: 'flex', minHeight: 400 }}>
        ...
        <div style={{ flex: '1', paddingLeft: 16, paddingRight: 16, borderRight: '1px solid #f0f0f0', display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          {sharedSearchesState.length > 0 ? (
            <div style={{ 
              ...
              >
                <SortableContext
                  items={sharedSearchesState.map((item) => item.id)}
                  strategy={verticalListSortingStrategy}
                >
                  <List
                    size="small"
                    dataSource={sharedSearchesState}
                    renderItem={(item) => (
        ...
        <div style={{ flex: '1', paddingLeft: 16, display: 'flex', flexDirection: 'column', height: '100%' }}>
          ...
          {personalSearchesState.length > 0 ? (
            <div style={{ 
              flex: 1, 
              overflowY: 'auto', 
              maxHeight: '400px',
              ...
              >
                <SortableContext
                  items={personalSearchesState.map((item) => item.id)}
                  strategy={verticalListSortingStrategy}
                >
                  <List
                    size="small"
                    dataSource={personalSearchesState}
                    renderItem={(item) => (
                      <SortableListItem id={item.id}>
                        {(listeners) => (
                        <List.Item
...
export const QuerySearchButton: React.FC<QuerySearchButtonProps> = ({
  ...
  // ⭐ 修复：使用 useMemo 稳定 allPinnedSearches 引用，避免无限循环
  const allPinnedSearches = useMemo(() => {
    const items = savedSearchesData?.items || [];
    return items.filter((item) => item.is_pinned);
  }, [savedSearchesData?.items]);
  
  // ⭐ 修复：使用 useMemo 直接计算排序后的钉住条件，避免 useState + useEffect 导致的无限循环
