...
export default function PickingsScreen() {
  const [list, setList] = useState<ProductionPicking[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [statusFilter, setStatusFilter] = useState<string | undefined>('待领料');

  const load = useCallback(async () => {
    try {
      const res = await getProductionPickings({
        status: statusFilter,
        limit: 50,
      });
      setList(res || []);
  ...
  const handleConfirm = (item: ProductionPicking) => {
    if (item.status !== '待领料') {
      Toast.fail('只有待领料状态才能确认');
      return;
    }
    Alert.alert('确认领料', `确认领料单 ${item.picking_code}？`, [
      { text: '取消', style: 'cancel' },
      {
        text: '确认',
        onPress: async () => {
          try {
            await confirmProductionPicking(item.id);
            Toast.success('领料确认成功');
            load();
          } catch (e: any) {
      ...
      >
        {loading ? (
          ...
          <View style={styles.emptyWrap}>
            ...
            <Text style={styles.emptyText}>暂无领料单</Text>
          </View>
        ) : (
          list.map((item) => (
            <View key={item.id} style={styles.card}>
              <View style={styles.cardRow}>
                <Text style={styles.cardTitle}>{item.picking_code}</Text>
                <Text style={[styles.cardStatus, item.status === '待领料' && styles.cardStatusPending]}>
              ...
              {item.status === '待领料' && (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={styles.confirmBtnText}>确认领料</Text>
                </TouchableOpacity>
              )}
            </View>
          ))
