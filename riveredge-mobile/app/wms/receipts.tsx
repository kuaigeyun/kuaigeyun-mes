...
export default function ReceiptsScreen() {
  const [list, setList] = useState<FinishedGoodsReceipt[]>([]);
  const [loading, setLoading] = useState(true);
  const [refreshing, setRefreshing] = useState(false);
  const [statusFilter, setStatusFilter] = useState<string | undefined>('待入库');
  ...
  const handleConfirm = (item: FinishedGoodsReceipt) => {
    if (item.status !== '待入库') {
      Toast.fail('只有待入库状态才能确认');
      return;
    }
    Alert.alert('确认入库', `确认入库单 ${item.receipt_code || item.id}？`, [
      { text: '取消', style: 'cancel' },
      {
        text: '确认',
        onPress: async () => {
          try {
            await confirmFinishedGoodsReceipt(item.id);
          ...
          list.map((item) => (
            <View key={item.id} style={styles.card}>
              <View style={styles.cardRow}>
                <Text style={styles.cardTitle}>{item.receipt_code || `入库单#${item.id}`}</Text>
                <Text style={[styles.cardStatus, item.status === '待入库' && styles.cardStatusPending]}>
              ...
              {item.status === '待入库' && (
                <TouchableOpacity
                  ...
                  activeOpacity={0.8}
                >
                  <Text style={styles.confirmBtnText}>确认入库</Text>
                </TouchableOpacity>
              )}
            </View>
          ))
