...
class Permission(BaseModel):
    ...
    用于管理 SaaS 多组织系统中的权限信息。
    所有权限必须属于某个组织（tenant_id），实现多组织数据隔离。
    同一组织内权限代码必须唯一。
    ...
    Attributes:
        ...
        name: 权限名称（组织内唯一）
        code: 权限代码（组织内唯一，用于程序识别，格式：resource:action）
        description: 权限描述（可选）
    ...
    class Meta:
        ...
        indexes = [
            ...
            ("resource",),            # 资源名称索引
            ("action",),              # 操作类型索引
            ("tenant_id", "code"),     # 组织 ID + 权限代码联合索引（组织内权限代码唯一）
        ]
        unique_together = [("tenant_id", "code")]  # 同一组织下权限代码唯一
