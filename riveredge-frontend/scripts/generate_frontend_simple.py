...
def generate_simple_page(module_name, ModelName, model_name, model_name_cn, api_path, no_field, no_field_cn):
    """生成简化的页面文件"""
    page_content = f'''/**
 * {model_name_cn}管理页面
 */
...
'''
        ...
        for ModelName, schema_file, model_name_cn, api_path in models:
            ...
            # 添加到types
            types_content += f'''export interface {ModelName} {{
  id: number;
  uuid: string;
  tenantId?: number;
  {no_field}: string;
...
''')
            ...
            page_content = generate_simple_page(module_name, ModelName, model_name, model_name_cn, api_path, no_field, no_field_cn)
