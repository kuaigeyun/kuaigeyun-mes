...
// ... (inside UserProfilePage component)
  ...
  const fetchBiometricDevices = async () => {
    try {
      const response = await apiRequest('/auth/biometric/credentials', { method: 'GET' });
      setBiometricDevices(response || []);
    } catch (error) {
      console.error('获取生物识别设备失败:', error);
    }
  };
  ...
  const handleAddBiometric = async () => {
    try {
      ...
      // 1. 获取注册选项
      const options = await apiRequest('/auth/biometric/register-options', { method: 'GET' });
      const publicKey = parseRegistrationOptions(options);

      // 2. 调用浏览器 API
      const credential = await navigator.credentials.create({ publicKey }) as PublicKeyCredential;
      if (!credential) throw new Error('并未创建凭据');
      ...
      const response = {
        ...
        response: {
          ...
          transports: (credential.response as any).getTransports ? (credential.response as any).getTransports() : [],
        },
      };

      // 4. 发送到后端完成注册
      await apiRequest('/auth/biometric/register-finalize', {
        method: 'POST',
        data: response,
      });

      messageApi.success('生物识别设备注册成功');
      fetchBiometricDevices();
    } catch (error: any) {
      console.error('注册生物识别失败:', error);
      messageApi.error(error.message || '注册失败，请确保您的设备支持并已开启生物识别');
    } finally {
  ...
  const handleDeleteBiometric = async (id: number) => {
    try {
      await apiRequest(`/auth/biometric/credentials/${id}`, { method: 'DELETE' });
  ...
  // ... (inside the Tabs items array, key: 'security')
                  children: (
                    <Space direction="vertical" style={{ width: '100%' }} size="large">
                      ...
                      <Card size="small" bordered={false} style={{ background: token.colorFillAlter }}>
                        <div style={{ marginBottom: 16 }}>
                          <Text type="secondary">
                            注册您的生物识别设备（如：指纹、面容 ID、TouchID），在下次登录时可以快速通过身份验证。
                          </Text>
                        </div>

                        {!biometricSupported ? (
                          <div style={{ color: token.colorWarning }}>
                            您的浏览器或设备当前不支持 WebAuthn 生物识别，请尝试更新浏览器或检查系统设置。
                          </div>
                        ) : (
                          <Space direction="vertical" style={{ width: '100' }}>
                            {biometricDevices.length > 0 && (
                              <div style={{ marginBottom: 16 }}>
                                <Title level={5}>已注册设备</Title>
                                {biometricDevices.map(device => (
                                  <div key={device.id} style={{ 
                                    ...
                                    <Space>
                                      <FingerprintOutlined />
                                      <Text>{device.display_name || '生物识别设备'}</Text>
                                      <Text type="secondary" size="small">
                                        (注册于 {new Date(device.created_at).toLocaleDateString()})
                                      </Text>
                                    </Space>
                                    <Button 
                                      type="text" 
                                      danger 
                                      icon={<DeleteOutlined />} 
                                      onClick={() => handleDeleteBiometric(device.id)}
                                    />
                                  </div>
                                ))}
                              </div>
