...
const SmartSuggestionFloatPanel: React.FC<SmartSuggestionFloatPanelProps> = ({
  ...
  const displayBlocks = useMemo(
    (): Array<{
      ...
      title: string;
      tone?: 'default' | 'danger';
      action?: 'apply' | 'revalidate' | 'custom';
      actionLabel?: string;
      onAction?: () => void;
    }> => {
      const blocks: Array<{
      ...
      if (messagesProp?.length) {
        return messagesProp.map((m) => ({
          ...
          title: m.title ?? '配置说明',
          tone: m.tone ?? 'default',
          action: m.onAction && m.actionLabel ? ('custom' as const) : undefined,
  ...
  const hasDangerTone = displayBlocks.some((b) => b.tone === 'danger');
  ...
  return createPortal(
    <div
      className={[
        'smart-suggestion-float-panel',
        isPositionReady ? 'smart-suggestion-float-panel--ready' : '',
        hasDangerTone ? 'smart-suggestion-float-panel--danger' : '',
      ]
        .filter(Boolean)
        .join(' ')}
      style={inlineStyle}
              ...
              (block, index) =>
                ...
                visibleBlockContents[index].length > 0 && (
                  <div
                    key={index}
                    className={[
                      'smart-suggestion-float-panel__block',
                      index > 0 ? 'smart-suggestion-float-panel__block--subsequent' : '',
                      block.tone === 'danger' ? 'smart-suggestion-float-panel__block--danger' : '',
                    ]
                      .filter(Boolean)
                      .join(' ')}
                  >
                ...
                {block.action === 'custom' && block.onAction && block.actionLabel && (
                  <div className="smart-suggestion-float-panel__block-actions">
                    <Button
                      size="small"
                      danger={block.tone === 'danger'}
