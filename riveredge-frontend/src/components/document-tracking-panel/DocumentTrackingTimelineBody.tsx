...
export const DocumentTrackingTimelineBody: React.FC<{
  ...
  const renderTimelineItem = (item: DocumentTrackingTimelineItem) => {
    ...
    const resolveDocTypeLabel = (documentType: string) =>
      t(`components.documentTrackingPanel.docType.${documentType}`, {
        defaultValue: documentType,
      });

    let relationDetailText = item.detail;
    if (
      (item.type === 'from' || item.type === 'pull') &&
      item.source_type
    ) {
      const code = (item.source_code || '').trim() || String(item.source_id ?? '');
      relationDetailText = t('components.documentTrackingPanel.detailFrom', {
        type: resolveDocTypeLabel(item.source_type),
        code,
      });
    } else if (item.type === 'push' && item.target_type) {
      const code = (item.target_code || '').trim() || String(item.target_id ?? '');
      relationDetailText = t('components.documentTrackingPanel.detailPush', {
        type: resolveDocTypeLabel(item.target_type),
        code,
      });
    }
