/**
 * 签名组件 - 基于 signature_pad 的手写签名
 *
 * 参考 pdfme-playground: https://github.com/pdfme/pdfme-playground
 */
...
const getEffectiveScale = (): number => 1; // 简化处理，避免昂贵的 DOM 回溯
...
export const signaturePlugin: Plugin<Schema> = {
  ui: async (props) => {
    ...
    const resetScale = 1 / getEffectiveScale();
    const ctx = canvas.getContext('2d');
    if (ctx) ctx.scale(resetScale, resetScale);
    ...
    if (mode === 'viewer') {
      ...
      signaturePad.addEventListener('endStroke', () => {
        ...
        if (data && onChange) onChange({ key: 'content', value: data });
      });
    }
