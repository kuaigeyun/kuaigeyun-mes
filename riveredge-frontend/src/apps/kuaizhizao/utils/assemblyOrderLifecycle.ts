...
function buildFallbackLifecycle(record: Record<string, unknown>): BackendLifecycle {
  ...
  const keyMap: Record<string, string> = { 草稿: 'draft', 组装中: 'in_progress', 已完成: 'completed', 已取消: 'cancelled' };
  ...
  const stageToIdx: Record<string, number> = { 草稿: 0, 组装中: 1, 已完成: 2, 已取消: 0 };
  ...
  return {
    ...
    status: isException ? 'exception' : stageName === '已完成' ? 'success' : stageName === '组装中' ? 'active' : 'normal',
    main_stages: mainStages,
    next_step_suggestions: stageName === '草稿' ? ['执行组装'] : stageName === '组装中' ? ['完成'] : [],
  };
}

export function getAssemblyOrderLifecycle(
