...
function buildFallbackLifecycle(record: Record<string, unknown>): BackendLifecycle {
  ...
  const stageName = (STATUS_TO_STAGE[status] ?? status) || '待收货';
  ...
  const stageToIdx: Record<string, number> = { 待收货: 0, 已通知: 1, 已入库: 2 };
  ...
  return {
    ...
    status: stageName === '已入库' ? 'success' : stageName === '已通知' ? 'active' : 'normal',
    main_stages: mainStages,
    next_step_suggestions: stageName === '待收货' ? ['通知仓库'] : stageName === '已通知' ? ['入库'] : [],
  };
}

export function getReceiptNoticeLifecycle(
