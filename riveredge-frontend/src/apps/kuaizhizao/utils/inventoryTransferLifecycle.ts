...
function buildFallbackLifecycle(record: Record<string, unknown>): BackendLifecycle {
  ...
  return {
    ...
    status: isException ? 'exception' : stageName === '已完成' ? 'success' : stageName === '调拨中' ? 'active' : 'normal',
    main_stages: mainStages,
    next_step_suggestions: stageName === '草稿' ? ['执行调拨'] : stageName === '调拨中' ? ['完成'] : [],
  };
}

export function getInventoryTransferLifecycle(
