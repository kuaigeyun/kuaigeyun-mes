...
function buildFallbackLifecycle(record: Record<string, unknown>): BackendLifecycle {
  ...
  const keyMap: Record<string, string> = { 草稿: 'draft', 盘点中: 'in_progress', 已完成: 'completed', 已取消: 'cancelled' };
  ...
  const stageToIdx: Record<string, number> = { 草稿: 0, 盘点中: 1, 已完成: 2, 已取消: 0 };
  ...
  return {
    ...
    status: isException ? 'exception' : stageName === '已完成' ? 'success' : stageName === '盘点中' ? 'active' : 'normal',
    main_stages: mainStages,
    next_step_suggestions: stageName === '草稿' ? ['开始盘点'] : stageName === '盘点中' ? ['完成盘点'] : [],
  };
}

export function getStocktakingLifecycle(
