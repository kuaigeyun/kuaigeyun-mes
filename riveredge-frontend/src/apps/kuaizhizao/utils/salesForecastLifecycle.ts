...
const SALES_FORECAST_STAGE_KEYS = new Set(['draft', 'pending_review', 'audited', 'pushed', 'rejected']);
...
function isSalesForecastLifecycle(backend: BackendLifecycle): boolean {
  ...
  return [...keys].some((k) => SALES_FORECAST_STAGE_KEYS.has(k));
}
...
function buildMainStages(currentKey: string): SubStage[] {
  ...
  const isCompleted = currentKey === '已下推';
