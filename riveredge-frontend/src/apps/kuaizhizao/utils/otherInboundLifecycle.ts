...
function buildFallbackLifecycle(record: Record<string, unknown>): BackendLifecycle {
  ...
  const stageName = (STATUS_TO_STAGE[status] ?? status) || '待入库';
  const keyMap: Record<string, string> = { 待入库: 'pending', 已入库: 'received', 已取消: 'cancelled' };
  ...
  const stageToIdx: Record<string, number> = { 待入库: 0, 已入库: 1, 已取消: 0 };
  ...
  return {
    ...
    status: isException ? 'exception' : stageName === '已入库' ? 'success' : 'normal',
    main_stages: mainStages,
    next_step_suggestions: stageName === '待入库' ? ['确认入库'] : [],
  };
}

export function getOtherInboundLifecycle(
