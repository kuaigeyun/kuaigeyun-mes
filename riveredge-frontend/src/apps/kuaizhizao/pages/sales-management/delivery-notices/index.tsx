...
const DeliveryNoticesPage: React.FC = () => {
  ...
  return (
    <>
        ...
        <UniTable
          ...
          onExport={async (type, keys, pageData) => {
            try {
              ...
              a.href = url;
              a.download = `delivery-notices-${new Date().toISOString().slice(0, 10)}.json`;
              a.click();
      ...
      <DetailDrawerTemplate
        title={`发货通知详情${noticeDetail?.notice_code ? ` - ${noticeDetail.notice_code}` : ''}`}
...
export default DeliveryNoticesPage;
