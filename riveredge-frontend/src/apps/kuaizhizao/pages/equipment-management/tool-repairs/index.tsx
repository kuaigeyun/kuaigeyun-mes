...
const ToolRepairsPage: React.FC = () => {
  ...
  const columns: ProColumns<ToolRepair>[] = useMemo(
    () => [
      ...
      { title: t(`${P}.col.repairDate`), dataIndex: 'repair_date', width: 110, valueType: 'date' },
      { title: t(`${P}.col.faultDescription`), dataIndex: 'fault_description', ellipsis: true },
      { title: t(`${P}.col.urgency`), dataIndex: 'urgency', width: 90 },
      {
      ...
      },
      {
        ...
        render: (_, record) => (
          <>
            ...
            {canAudit && (record.status === '待审批' || record.status === '已提交') && (
              <Button
    ...
    <>
      <ListPageTemplate>
        <UniTable<ToolRepair>
          ...
          columnPersistenceId="apps.kuaizhizao.pages.equipment-management.tool-repairs"
          actionRef={actionRef}
          rowKey="id"
          columns={columns}
