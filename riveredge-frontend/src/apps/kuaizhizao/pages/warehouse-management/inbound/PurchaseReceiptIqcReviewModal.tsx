...
type Props = {
  open: boolean;
  loading?: boolean;
  purchaseReceiptId?: number | string;
  ensure: EnsureIqcForPurchaseReceiptResult | null;
  t: TFunction;
...
export const PurchaseReceiptIqcReviewModal: React.FC<Props> = ({
  open,
  loading,
  purchaseReceiptId,
  ensure,
    ...
    <Modal
      ...
      footer={
        <Space>
          ...
          {purchaseReceiptId != null && ensure?.requires_iqc && !canConfirm ? (
            <Button
              type="primary"
              onClick={() => {
                onCancel();
          ...
          <Button type="primary" disabled={!canConfirm} onClick={onContinue}>
            {t('app.kuaizhizao.warehouseInbound.iqcReview.continueConfirm')}
          </Button>
        </Space>
      }
    >
      <Space orientation="vertical" size={12} style={{ width: '100%' }}>
        {ensure?.created_count ? (
          <Alert
            type="info"
            showIcon
            title={t('app.kuaizhizao.warehouseInbound.iqc.autoCreated', { count: ensure.created_count })}
          />
        ) : null}
        ...
        <Descriptions size="small" column={2} bordered>
          ...
          <Descriptions.Item label={t('app.kuaizhizao.warehouseInbound.iqcReview.gate')}>
            {ensure?.gate_enabled
              ? t('app.kuaizhizao.warehouseInbound.iqcReview.gateOn')
              : t('app.kuaizhizao.warehouseInbound.iqcReview.gateOff')}
          </Descriptions.Item>
        </Descriptions>
        <Typography.Text type="secondary">{t('app.kuaizhizao.warehouseInbound.iqcReview.hint')}</Typography.Text>
        <Table
          size="small"
