...
import { listDrivers, type Driver } from '../../../services/logistics';
import { DriverFormModal } from './DriverFormModal';

export type DriverSelectDropdownProps = Omit<
  UniDropdownProps,
...
export const DriverSelectDropdown: React.FC<DriverSelectDropdownProps> = ({
  modalZIndex,
  onChange,
  value,
  ...rest
  ...
  const refresh = useCallback(async () => {
    setLoading(true);
    try {
      const res = await listDrivers({ limit: 200 });
      setItems(res.items);
      return res.items;
    } catch (error) {
    ...
    <>
      <UniDropdown
        ...
        quickCreate={
          perms.canCreate
            ? {
                label: t('app.kuaizhizao.logistics.action.quickCreateDriver'),
                onClick: () => {
                  setEditing(null);
                  setFormOpen(true);
                },
        ...
        quickEdit={
          perms.canUpdate
            ? {
                label: t('app.kuaizhizao.logistics.action.editDriver'),
                onEdit: (id) => {
