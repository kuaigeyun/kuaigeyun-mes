...
import { kpiApi } from '../../services/process';
import type { KPI, KPICreate, KPIUpdate } from '../../types/process';
...
const KPIPage: React.FC = () => {
  ...
  const handleEdit = async (record: KPI) => {
    try {
      setIsEdit(true);
      setCurrentUuid(record.uuid);
      setModalVisible(true);
      const data = await kpiApi.get(record.uuid);
  ...
  const handleDelete = async (record: KPI) => {
    try {
      await kpiApi.delete(record.uuid);
  ...
  const handleSubmit = async (values: KPICreate | KPIUpdate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await kpiApi.update(currentUuid, values as KPIUpdate);
        messageApi.success('更新成功');
      } else {
        await kpiApi.create(values as KPICreate);
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<KPI>
        ...
        request={async (params) => {
          ...
          const data = await kpiApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑KPI' : '新建KPI'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="kpiCode" label="KPI编码" rules={[{ required: true }]} disabled={isEdit} />
