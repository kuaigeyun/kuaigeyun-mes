...
import { budgetApi } from '../../services/process';
import type { Budget, BudgetCreate, BudgetUpdate } from '../../types/process';
...
const BudgetPage: React.FC = () => {
  ...
  const handleEdit = async (record: Budget) => {
    try {
      setIsEdit(true);
      setCurrentUuid(record.uuid);
      setModalVisible(true);
      const data = await budgetApi.get(record.uuid);
  ...
  const handleDelete = async (record: Budget) => {
    try {
      await budgetApi.delete(record.uuid);
  ...
  const handleSubmit = async (values: BudgetCreate | BudgetUpdate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await budgetApi.update(currentUuid, values as BudgetUpdate);
        messageApi.success('更新成功');
      } else {
        await budgetApi.create(values as BudgetCreate);
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<Budget>
        ...
        request={async (params) => {
          ...
          const data = await budgetApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑预算' : '新建预算'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="budgetNo" label="预算编号" rules={[{ required: true }]} disabled={isEdit} />
