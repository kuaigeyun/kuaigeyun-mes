...
import {
  addMoldUpkeepParamSetItem,
  createMoldUpkeepParamSetWithItems,
  deleteMoldUpkeepParamSet,
  deleteMoldUpkeepParamSetItem,
  listMoldUpkeepParamSetItems,
  listMoldUpkeepParamSets,
  listMoldUpkeepParams,
  updateMoldUpkeepParamSet,
  updateMoldUpkeepParamSetItem,
  type MoldUpkeepParamRow,
  type MoldUpkeepParamSetItemRow,
...
type LineRow = MoldUpkeepParamSetItemRow & {
  param_code: string;
  param_name: string;
...
const MoldUpkeepParamSetsPage: React.FC = () => {
  ...
  const [allParams, setAllParams] = useState<MoldUpkeepParamRow[]>([]);
  const [addItemOpen, setAddItemOpen] = useState(false);
  const [addItemLoading, setAddItemLoading] = useState(false);
  const [detailOpen, setDetailOpen] = useState(false);
  const [detailRecord, setDetailRecord] = useState<MoldUpkeepParamSetRow | null>(null);
  const [detailLines, setDetailLines] = useState<LineRow[]>([]);
  const [detailLoading, setDetailLoading] = useState(false);

  const loadParams = useCallback(async () => {
    const params = await listMoldUpkeepParams();
    setAllParams(params);
    return params;
  }, []);
  ...
  const loadLines = useCallback(
    async (setId: number) => {
      ...
      try {
        const [params, rawItems] = await Promise.all([listMoldUpkeepParams(), listMoldUpkeepParamSetItems(setId)]);
        setAllParams(params);
        ...
        messageApi.error((e as Error).message || t('app.haoligo.molds.upkeepParamSets.loadLinesFailed'));
        setLines([]);
      } finally {
  ...
  const openEditEditor = async (record: MoldUpkeepParamSetRow) => {
  ...
  const handleDetail = async (record: MoldUpkeepParamSetRow) => {
    setDetailRecord(record);
    setDetailOpen(true);
    setDetailLoading(true);
    try {
      const [params, rawItems] = await Promise.all([listMoldUpkeepParams(), listMoldUpkeepParamSetItems(record.id)]);
      setDetailLines(
        rawItems
          .map((it) => {
            const p = params.find((x) => x.id === it.param_id);
  ...
  const saveEditor = async () => {
    ...
    if (!name) {
      messageApi.warning(t('app.haoligo.molds.upkeepParamSets.nameRequired'));
      return;
    }
    if (editorCreateMode) {
      const code = String(v.code ?? '').trim();
      if (!code) {
        messageApi.warning(t('app.haoligo.molds.upkeepParamSets.codeRequired'));
        return;
      }
      if (pendingLines.length === 0) {
        messageApi.warning(t('app.haoligo.molds.upkeepParamSets.atLeastOneItem'));
        return;
      }
      setEditorSaving(true);
      try {
        await createMoldUpkeepParamSetWithItems({
          code,
          name,
    ...
    setEditorSaving(true);
    try {
      await updateMoldUpkeepParamSet(editorSetId, { name });
  ...
  const openAddItem = () => {
    if (!addableParamOptions.length) {
      messageApi.info(t('app.haoligo.molds.upkeepParamSets.noParamsToAdd'));
      return;
    }
    setAddItemOpen(true);
    setTimeout(() => {
  ...
  const handleAddItemSubmit = async (values: Record<string, unknown>) => {
    ...
    if (!paramIds.length) {
      messageApi.warning(t('app.haoligo.molds.upkeepParamSets.pickParamRequired'));
      return;
    }
    ...
    try {
      await Promise.all(
        paramIds.map((param_id, i) =>
          addMoldUpkeepParamSetItem(editorSetId, {
            param_id,
            sort_order: sortStart + i,
            is_required: isRequired,
          }),
  ...
  const columns: ProColumns<MoldUpkeepParamSetRow>[] = [
    { title: t('app.haoligo.molds.upkeepParamSets.colSetCode'), dataIndex: 'code', width: 140 },
    { title: t('app.haoligo.molds.upkeepParamSets.colSetName'), dataIndex: 'name', ellipsis: true },
    {
      ...
      render: (_, record) => (
        <Space>
          ...
          <Button key="delete" {...rowActionKind('delete')}
            ...
            onClick={() => {
              Modal.confirm({
                title: t('app.haoligo.molds.upkeepParamSets.deleteTitle'),
                content: record.name,
                okType: 'danger',
                onOk: async () => {
                  await deleteMoldUpkeepParamSet(record.id);
  ...
  const lineColumns: ColumnsType<LineRow> = [
    ...
    { title: t('app.haoligo.molds.upkeepParams.colCode'), dataIndex: 'param_code', width: 120 },
    { title: t('app.haoligo.molds.upkeepParams.colName'), dataIndex: 'param_name', ellipsis: true },
    {
      ...
      render: (_, r) =>
        normalizeMoldUpkeepValueType(r.value_type) === 'multiselect'
          ? t('app.haoligo.molds.upkeepParams.valueTypeMultiselect')
          : t('app.haoligo.molds.upkeepParams.valueTypeText'),
    },
    {
      title: t('app.haoligo.molds.upkeepParamSets.colRequired'),
      ...
      render: (_, r) => (
        <Switch
          ...
          onChange={async (checked) => {
            await updateMoldUpkeepParamSetItem(r.id, { is_required: checked });
    ...
    },
    {
      ...
      render: (_, r) => (
        <Popconfirm
          title={t('app.haoligo.molds.upkeepParamSets.removeConfirm')}
          onConfirm={async () => {
            await deleteMoldUpkeepParamSetItem(r.id);
            if (editorSetId) await loadLines(editorSetId);
          }}
        >
  ...
  return (
    <>
      <ListPageTemplate>
        <UniTable<MoldUpkeepParamSetRow>
          headerTitle={t('app.haoligo.menu.molds.upkeep-param-sets')}
          actionRef={actionRef}
          columns={columns}
          columnPersistenceId="apps.haoligo.pages.molds.upkeep-param-sets"
          showCreateButton
          onCreate={() => void openCreateEditor()}
          request={async () => {
            const data = await listMoldUpkeepParamSets();
      ...
      <Modal
        title={editorCreateMode ? t('app.haoligo.molds.upkeepParamSets.editorNew') : t('app.haoligo.molds.upkeepParamSets.editorEdit', { code: editorSetCode })}
        open={editorOpen}
      ...
      >
        <ProForm formRef={editorFormRef} submitter={false} layout="vertical">
          <Row gutter={16}>
            <Col span={12}>
              <ProFormText name="code" label={t('app.haoligo.molds.upkeepParamSets.colSetCode')} disabled={!editorCreateMode} rules={[{ required: editorCreateMode }]} />
            </Col>
            <Col span={12}>
              <ProFormText name="name" label={t('app.haoligo.molds.upkeepParamSets.colSetName')} rules={[{ required: true }]} />
            </Col>
          </Row>
        </ProForm>
        <Divider />
        <Space style={{ marginBottom: 8 }}>
          <Title level={5} style={{ margin: 0 }}>
            {t('app.haoligo.molds.upkeepParamSets.itemsTitle')}
          </Title>
          {!editorCreateMode && editorSetId != null ? (
            <Button type="primary" size="small" icon={<PlusOutlined />} onClick={openAddItem}>
              {t('app.haoligo.molds.upkeepParamSets.addItem')}
            </Button>
          ) : null}
          <Link to="/apps/haoligo/molds/upkeep-params">{t('app.haoligo.menu.molds.upkeep-params')}</Link>
        </Space>
        {editorCreateMode ? (
          <Table
            size="small"
            ...
            columns={[
              { title: '#', width: 48, render: (_, __, i) => i + 1 },
              { title: t('app.haoligo.molds.upkeepParams.colCode'), dataIndex: 'param_code' },
              { title: t('app.haoligo.molds.upkeepParams.colName'), dataIndex: 'param_name' },
              {
                title: t('app.haoligo.molds.upkeepParamSets.colRequired'),
                dataIndex: 'is_required',
                render: (_, r) => (
      ...
      <FormModalTemplate
        title={t('app.haoligo.molds.upkeepParamSets.addItem')}
        open={addItemOpen}
      ...
      >
        <ProFormSelect
          name="param_ids"
          label={t('app.haoligo.molds.upkeepParamSets.pickParam')}
          placeholder={t('app.haoligo.molds.upkeepParamSets.pickParamPh')}
          options={addableParamOptions}
          rules={[{ required: true, message: t('app.haoligo.molds.upkeepParamSets.pickParamRequired') }]}
          showSearch
          fieldProps={{
            mode: 'multiple',
        ...
        />
        <ProFormSwitch name="is_required" label={t('app.haoligo.molds.upkeepParamSets.colRequired')} />
      </FormModalTemplate>
...
export default MoldUpkeepParamSetsPage;
