...
import { healthRecordApi } from '../../services/process';
import type { HealthRecord, HealthRecordCreate, HealthRecordUpdate } from '../../types/process';
...
const HealthRecordPage: React.FC = () => {
  ...
  const handleEdit = async (record: HealthRecord) => {
    try {
      ...
      setModalVisible(true);
      const data = await healthRecordApi.get(record.uuid);
  ...
  const handleDelete = async (record: HealthRecord) => {
    try {
      await healthRecordApi.delete(record.uuid);
  ...
  const handleSubmit = async (values: HealthRecordCreate | HealthRecordUpdate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await healthRecordApi.update(currentUuid, values as HealthRecordUpdate);
        messageApi.success('更新成功');
      } else {
        await healthRecordApi.create(values as HealthRecordCreate);
  ...
  const columns: ProColumns<HealthRecord>[] = [
    {
      title: '档案编号',
      dataIndex: 'recordNo',
      width: 200,
  ...
  return (
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<HealthRecord>
        actionRef={actionRef}
        columns={columns}
        request={async (params) => {
          const { current = 1, pageSize = 20, ...filters } = params;
          const skip = (current - 1) * pageSize;
          const data = await healthRecordApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑健康档案' : '新建健康档案'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="recordNo" label="档案编号" rules={[{ required: true }]} disabled={isEdit} />
