...
import { regulationApi } from '../../services/process';
import type { Regulation, RegulationCreate, RegulationUpdate } from '../../types/process';
...
const RegulationPage: React.FC = () => {
  ...
  const handleEdit = async (record: Regulation) => {
    try {
      setIsEdit(true);
      setCurrentUuid(record.uuid);
      setModalVisible(true);
      const data = await regulationApi.get(record.uuid);
  ...
  const handleDelete = async (record: Regulation) => {
    try {
      await regulationApi.delete(record.uuid);
  ...
  const handleSubmit = async (values: RegulationCreate | RegulationUpdate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await regulationApi.update(currentUuid, values as RegulationUpdate);
        messageApi.success('更新成功');
      } else {
        await regulationApi.create(values as RegulationCreate);
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<Regulation>
        ...
        request={async (params) => {
          ...
          const data = await regulationApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑法规' : '新建法规'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="regulationNo" label="法规编号" rules={[{ required: true }]} disabled={isEdit} />
