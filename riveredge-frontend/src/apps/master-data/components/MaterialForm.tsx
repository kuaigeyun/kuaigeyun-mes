...
export const MaterialForm: React.FC<MaterialFormProps> = ({
  ...
  /** 最后一次自动生成的编码（用于提交时判断是否让后端生成，确保隔离字段生效） */
  ...
  const generateCode = useCallback(async (groupId?: number, sourceType?: string, name?: string, forceUpdate: boolean = false) => {
    ...
    // 构建上下文（用于编码规则，隔离字段 scope_fields 依赖 group_code）
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      // 启用自动编码且当前编码为系统自动生成时，不传 main_code 让后端生成，确保隔离字段（scope_fields）生效
      const isAutoGen = !isEdit && isAutoGenerateEnabled('master-data-material');
      const isCodeFromAutoGen = lastAutoGeneratedCodeRef.current != null && restValues.mainCode === lastAutoGeneratedCodeRef.current;
      const shouldBackendGenerateCode = isAutoGen && isCodeFromAutoGen;
      const submitData: any = {
        // 基础字段转换（驼峰 -> 蛇形）
        main_code: shouldBackendGenerateCode ? undefined : restValues.mainCode,
  ...
  return (
    <>
      ...
      <FormModalTemplate
        ...
        modalRender={(modal) => (
          <div data-smart-suggestion-anchor="material-form" style={{ overflow: 'hidden' }}>
            {modal}
          </div>
        )}
      >
