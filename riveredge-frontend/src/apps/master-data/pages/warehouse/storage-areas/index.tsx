...
const StorageAreasPage: React.FC = () => {
  ...
  const handleCreate = async () => {
    ...
    // 检查是否启用自动编码
    if (isAutoGenerateEnabled('master-data-warehouse-storage-area')) {
      const ruleCode = getPageRuleCode('master-data-warehouse-storage-area');
      if (ruleCode) {
        try {
          // 使用测试生成（不更新序号），仅用于预览
  ...
  const handleSubmit = async (values: any) => {
    try {
      ...
      if (isEdit && currentStorageAreaUuid) {
        ...
        if (!isEdit && isAutoGenerateEnabled('master-data-warehouse-storage-area')) {
          const ruleCode = getPageRuleCode('master-data-warehouse-storage-area');
  ...
  const columns: ProColumns<StorageArea>[] = [
    ...
    },
    {
      ...
      width: 200,
      hideInSearch: true,
      render: (_, record) => getWarehouseName(record?.warehouseId ?? (record as any)?.warehouse_id),
    },
    {
      title: '描述',
