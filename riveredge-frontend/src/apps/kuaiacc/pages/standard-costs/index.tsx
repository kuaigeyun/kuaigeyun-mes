...
import { standardCostApi } from '../../services/process';
import type { StandardCost, StandardCostCreate } from '../../types/process';
...
const StandardCostsPage: React.FC = () => {
  ...
  const [detail, setDetail] = useState<StandardCost | null>(null);
  ...
  const handleEdit = async (record: StandardCost) => {
    try {
      setIsEdit(true);
      setCurrentUuid(record.uuid);
      setModalVisible(true);
      const data = await standardCostApi.get(record.uuid);
  ...
  const handleDelete = async (record: StandardCost) => {
    try {
      await standardCostApi.delete(record.uuid);
  ...
  const handleOpenDetail = async (record: StandardCost) => {
    try {
      setCurrentUuid(record.uuid);
  ...
  const handleSubmit = async (values: StandardCostCreate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await standardCostApi.update(currentUuid, values);
        messageApi.success('更新成功');
      } else {
        await standardCostApi.create(values);
  ...
  const columns: ProColumns<StandardCost>[] = [
    ...
    },
    {
      ...
      valueEnum: {
        ...
        '已生效': { text: <Tag color="green">已生效</Tag> },
        '已失效': { text: <Tag color="red">已失效</Tag> },
      },
    },
    {
      title: '操作',
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<StandardCost>
        ...
        request={async (params) => {
          ...
          const data = await standardCostApi.list({ skip, limit: pageSize, ...rest });
          return { data, success: true, total: data.length };
        }}
        rowKey="uuid"
        search={{ labelWidth: 'auto' }}
        toolBarRender={() => [
          <Button type="primary" key="primary" icon={<PlusOutlined />} onClick={handleCreate}>新建标准成本</Button>,
        ]}
        rowSelection={{ selectedRowKeys, onChange: setSelectedRowKeys }}
      />
      <Modal title={isEdit ? '编辑标准成本' : '新建标准成本'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={800}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: isEdit ? '更新' : '创建' }, submitButtonProps: { loading: formLoading } }}>
          ...
          <ProFormDatePicker name="effectiveDate" label="生效日期" rules={[{ required: true }]} />
      ...
      <Drawer title="标准成本详情" open={drawerVisible} onClose={() => setDrawerVisible(false)} width={600}>
        {detailLoading ? <div>加载中...</div> : detail ? <ProDescriptions<StandardCost> column={1} dataSource={detail} /> : null}
      </Drawer>
    </div>
  );
};
