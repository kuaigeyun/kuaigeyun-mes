...
const InvoiceDetail: React.FC = () => {
  ...
  const handleFinish = async (values: any) => {
    ...
    let calculatedTotal = 0, calculatedTax = 0, calculatedExcl = 0;
  ...
  const columns: ProColumns<InvoiceItem>[] = [
    { title: '货物或应税劳务名称', dataIndex: 'item_name', width: '20%', formItemProps: { rules: [{ required: true, message: '此项为必填项' }] } },
    ...
    { title: '金额', dataIndex: 'amount', valueType: 'money', width: '10%', formItemProps: { rules: [{ required: true, message: '必填' }] }, editable: true },
    { title: '税率', dataIndex: 'tax_rate', valueType: 'percent', width: '10%', initialValue: 0.13 },
    { title: '税额', dataIndex: 'tax_amount', valueType: 'money', width: '10%', editable: false, render: (_, row: InvoiceItem) => (row.amount && row.tax_rate) ? (Number(row.amount) * Number(row.tax_rate)).toFixed(2) : row.tax_amount },
    { title: '操作', valueType: 'option', width: 100, render: (_, record) => [<a key="delete" onClick={() => setDataSource(dataSource.filter((item) => item.id !== record.id))}>删除</a>] },
  ];

  return (
    <PageContainer title={isCreate ? '新建发票' : '编辑发票'} loading={loading}>
      <ProForm form={form} onFinish={handleFinish} initialValues={{ invoice_date: dayjs(), category: 'IN', invoice_type: 'VAT_SPECIAL', tax_rate: 0.13 }}>
        <Card title="基础信息" bordered={false}>
          <ProForm.Group>
            <ProFormText name="invoice_code" label="系统编号" disabled placeholder="自动生成" width="md" />
            <ProFormSelect name="category" label="业务类型" options={[{ label: '进项(采购)', value: 'IN' }, { label: '销项(销售)', value: 'OUT' }]} width="md" rules={[{ required: true }]} />
            ...
            <ProFormSelect name="invoice_type" label="发票类型" options={[{ label: '增值税专用发票', value: 'VAT_SPECIAL' }, { label: '增值税普通发票', value: 'VAT_NORMAL' }, { label: '电子发票', value: 'ELECTRONIC' }]} width="md" />
        ...
        <Card title="发票明细" bordered={false} style={{ marginTop: 16 }}>
          <EditableProTable<InvoiceItem>
            ...
            recordCreatorProps={{ newRecordType: 'dataSource', record: () => ({ id: Date.now(), item_name: '', amount: 0, tax_rate: 0.13, tax_amount: 0 }) }}
            columns={columns}
            editable={{
              type: 'multiple',
              editableKeys,
        ...
        <Card title="合计信息" bordered={false} style={{ marginTop: 16 }}>
          <ProFormDependency name={['items']}>
            {() => {
              let total = 0, tax = 0;
              dataSource.forEach(i => { total += Number(i.amount || 0) + Number(i.tax_amount || 0); tax += Number(i.tax_amount || 0); });
              return (
                <Space size="large">
