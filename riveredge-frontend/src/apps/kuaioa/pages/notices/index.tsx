...
import { noticeApi } from '../../services/process';
import type { Notice, NoticeCreate, NoticeUpdate } from '../../types/process';
...
const NoticePage: React.FC = () => {
  ...
  const handleEdit = async (record: Notice) => {
    try {
      setIsEdit(true);
      setCurrentUuid(record.uuid);
      setModalVisible(true);
      const data = await noticeApi.get(record.uuid);
  ...
  const handleDelete = async (record: Notice) => {
    try {
      await noticeApi.delete(record.uuid);
  ...
  const handleSubmit = async (values: NoticeCreate | NoticeUpdate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await noticeApi.update(currentUuid, values as NoticeUpdate);
        messageApi.success('更新成功');
      } else {
        await noticeApi.create(values as NoticeCreate);
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<Notice>
        ...
        request={async (params) => {
          ...
          const data = await noticeApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑公告' : '新建公告'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="noticeNo" label="公告编号" rules={[{ required: true }]} disabled={isEdit} />
