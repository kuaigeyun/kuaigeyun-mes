...
import { approvalProcessApi } from '../../services/process';
import type { ApprovalProcess, ApprovalProcessCreate, ApprovalProcessUpdate } from '../../types/process';
...
const ApprovalProcessPage: React.FC = () => {
  ...
  const handleEdit = async (record: ApprovalProcess) => {
    try {
      ...
      setModalVisible(true);
      const data = await approvalProcessApi.get(record.uuid);
  ...
  const handleDelete = async (record: ApprovalProcess) => {
    try {
      await approvalProcessApi.delete(record.uuid);
  ...
  const handleSubmit = async (values: ApprovalProcessCreate | ApprovalProcessUpdate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await approvalProcessApi.update(currentUuid, values as ApprovalProcessUpdate);
        messageApi.success('更新成功');
      } else {
        await approvalProcessApi.create(values as ApprovalProcessCreate);
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<ApprovalProcess>
        ...
        request={async (params) => {
          ...
          const data = await approvalProcessApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑审批流程' : '新建审批流程'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="processNo" label="流程编号" rules={[{ required: true }]} disabled={isEdit} />
