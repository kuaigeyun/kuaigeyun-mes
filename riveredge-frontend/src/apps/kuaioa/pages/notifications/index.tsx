...
import { notificationApi } from '../../services/process';
import type { Notification, NotificationCreate, NotificationUpdate } from '../../types/process';
...
const NotificationPage: React.FC = () => {
  ...
  const handleEdit = async (record: Notification) => {
    try {
      ...
      setModalVisible(true);
      const data = await notificationApi.get(record.uuid);
  ...
  const handleDelete = async (record: Notification) => {
    try {
      await notificationApi.delete(record.uuid);
  ...
  const handleSubmit = async (values: NotificationCreate | NotificationUpdate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await notificationApi.update(currentUuid, values as NotificationUpdate);
        messageApi.success('更新成功');
      } else {
        await notificationApi.create(values as NotificationCreate);
  ...
  return (
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<Notification>
        actionRef={actionRef}
        columns={columns}
        request={async (params) => {
          const { current = 1, pageSize = 20, ...filters } = params;
          const skip = (current - 1) * pageSize;
          const data = await notificationApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑通知' : '新建通知'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="notificationNo" label="通知编号" rules={[{ required: true }]} disabled={isEdit} />
