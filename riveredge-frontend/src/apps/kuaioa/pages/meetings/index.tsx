...
import { meetingApi } from '../../services/process';
import type { Meeting, MeetingCreate, MeetingUpdate } from '../../types/process';
...
const MeetingPage: React.FC = () => {
  ...
  const handleEdit = async (record: Meeting) => {
    try {
      setIsEdit(true);
      setCurrentUuid(record.uuid);
      setModalVisible(true);
      const data = await meetingApi.get(record.uuid);
  ...
  const handleDelete = async (record: Meeting) => {
    try {
      await meetingApi.delete(record.uuid);
  ...
  const handleSubmit = async (values: MeetingCreate | MeetingUpdate) => {
    try {
      setFormLoading(true);
      if (isEdit && currentUuid) {
        await meetingApi.update(currentUuid, values as MeetingUpdate);
        messageApi.success('更新成功');
      } else {
        await meetingApi.create(values as MeetingCreate);
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<Meeting>
        ...
        request={async (params) => {
          ...
          const data = await meetingApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑会议' : '新建会议'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="meetingNo" label="会议编号" rules={[{ required: true }]} disabled={isEdit} />
