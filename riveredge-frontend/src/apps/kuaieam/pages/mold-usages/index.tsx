 ...
 * 模具使用管理页面
 * 
 * 提供模具使用记录的 CRUD 功能，包括列表展示、创建、编辑、删除等操作。
 */
...
import { moldUsageApi } from '../../services/process';
import type { MoldUsage, MoldUsageCreate, MoldUsageUpdate } from '../../types/process';
...
const MoldUsagesPage: React.FC = () => {
  ...
  const [usageDetail, setUsageDetail] = useState<MoldUsage | null>(null);
  const [detailLoading, setDetailLoading] = useState(false);
  
  // Modal 相关状态（创建/编辑模具使用）
   ...
   * 处理编辑模具使用
   */
  const handleEdit = async (record: MoldUsage) => {
    try {
      setIsEdit(true);
      setCurrentUsageUuid(record.uuid);
      setModalVisible(true);
      
      const detail = await moldUsageApi.get(record.uuid);
      formRef.current?.setFieldsValue({
      ...
      messageApi.error(error.message || '获取模具使用详情失败');
    }
  };
   ...
   * 处理删除模具使用
   */
  const handleDelete = async (record: MoldUsage) => {
    try {
      await moldUsageApi.delete(record.uuid);
   ...
   * 处理打开详情
   */
  const handleOpenDetail = async (record: MoldUsage) => {
    try {
      setCurrentUsageUuid(record.uuid);
   ...
   * 处理提交表单
   */
  const handleSubmit = async (values: MoldUsageCreate | MoldUsageUpdate) => {
    try {
      setFormLoading(true);
      
      if (isEdit && currentUsageUuid) {
        await moldUsageApi.update(currentUsageUuid, values as MoldUsageUpdate);
        messageApi.success('更新成功');
      } else {
        await moldUsageApi.create(values as MoldUsageCreate);
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<MoldUsage>
        ...
        request={async (params) => {
          ...
          const data = await moldUsageApi.list({
            skip,
            limit: pageSize,
            ...rest,
          });
      ...
      >
        ...
        >
          ...
          <ProFormDigit
            name="moldId"
            label="模具ID"
            rules={[{ required: true, message: '请输入模具ID' }]}
            placeholder="请输入模具ID"
          />
          <ProFormText
            name="moldName"
      ...
      >
        {detailLoading ? (
          ...
          <ProDescriptions<MoldUsage>
            column={1}
            dataSource={usageDetail}
            columns={[
              { title: '使用记录编号', dataIndex: 'usageNo' },
              { title: '模具ID', dataIndex: 'moldId' },
              { title: '模具名称', dataIndex: 'moldName' },
              { title: '模具编码', dataIndex: 'moldCode' },
