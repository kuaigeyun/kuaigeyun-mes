...
const RealTimeMonitoringPage: React.FC = () => {
  ...
  const handleEdit = async (record: RealTimeMonitoring) => {
    try {
      ...
      setModalVisible(true);
      const data = await realTimeMonitoringApi.get(record.uuid);
  ...
  const handleSubmit = async (values: RealTimeMonitoringCreate | RealTimeMonitoringUpdate) => {
    try {
      ...
      if (isEdit && currentUuid) {
        await realTimeMonitoringApi.update(currentUuid, values as RealTimeMonitoringUpdate);
  ...
  const columns: ProColumns<RealTimeMonitoring>[] = [
    ...
    },
    {
      ...
      width: 200,
      fixed: 'right',
      render: (_: any, record: RealTimeMonitoring) => (
        <Space>
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<RealTimeMonitoring>
        ...
        request={async (params) => {
          ...
          const data = await realTimeMonitoringApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑实时监控' : '新建实时监控'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
