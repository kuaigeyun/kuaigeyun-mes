...
const SensorDataPage: React.FC = () => {
  ...
  const handleEdit = async (record: SensorData) => {
    try {
      ...
      setModalVisible(true);
      const data = await sensorDataApi.get(record.uuid);
  ...
  const handleSubmit = async (values: SensorDataCreate | SensorDataUpdate) => {
    try {
      ...
      if (isEdit && currentUuid) {
        await sensorDataApi.update(currentUuid, values as SensorDataUpdate);
    ...
    <div style={{ padding: '16px', margin: 0, boxSizing: 'border-box' }}>
      <UniTable<SensorData>
        ...
        request={async (params) => {
          ...
          const data = await sensorDataApi.list({ skip, limit: pageSize, ...filters });
      ...
      />
      <Modal title={isEdit ? '编辑传感器数据' : '新建传感器数据'} open={modalVisible} onCancel={() => setModalVisible(false)} footer={null} width={600}>
        <ProForm formRef={formRef} onFinish={handleSubmit} submitter={{ searchConfig: { submitText: '确定' }, submitButtonProps: { loading: formLoading } }}>
          <ProFormText name="dataNo" label="数据编号" rules={[{ required: true }]} disabled={isEdit} />
